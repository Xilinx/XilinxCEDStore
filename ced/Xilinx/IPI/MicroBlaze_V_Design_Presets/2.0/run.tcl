...
proc mbv_clkwiz_reset_pin {clkwiz_cell} {
    # Clocking Wizard's CONFIG.RESET_PORT changes the pin's actual name
    # (reset vs resetn) depending on the connected board's clock preset
    # (e.g. SCU200/SCU35 sys_diff_clock_preset sets RESET_PORT=resetn).
    # Resolve whichever one actually exists instead of assuming "reset".
    set p [get_bd_pins -quiet ${clkwiz_cell}/reset]
    if {$p eq ""} { set p [get_bd_pins -quiet ${clkwiz_cell}/resetn] }
    return $p
}
...
proc create_root_design {parentCell design_name options} {
    ...
    # init.tcl exposes "Linux_Processor" as the canonical value but users
    # sometimes pass "Linux_Profile", "linux_processor", etc.
    switch -nocase -glob $preset {
        ...
        "Linux_Processor"       -
        "Linux_Profile"         -
        "Linux*"                { set preset "Linux_Processor"     }
    }
    ...
    # Detect the 4GB memory part variant (MT62F1G32D2DS-020 WT:D, 32Gb).
    # This variant requires base address 0x100000000 and 64-bit DMA addressing
    # because its address space starts beyond the 32-bit boundary.
    set lpddrmc_4g_ifc [board::get_board_part_interfaces lpddr5_sdram_4g]
    if {$lpddrmc_4g_ifc ne ""} {
        set lpddrmc_board_interface_1 lpddr5_sdram_4g
        set mbv_is_lpddrmc_4g 1
    } else {
        set mbv_is_lpddrmc_4g 0
    }
    ...
    if {$preset eq "Microcontroller"} {
        ...
        if {$mbv_spartan_us_plus} {
            ...
            # sys_clock must be connected first: it fires the board's sys_diff_clock_preset,
            # which is what sets clk_wiz_1's CONFIG.RESET_PORT (reset vs resetn) on SpartanUS+
            # boards. Resolving/wiring the reset pin before this leaves it unnamed, which sends
            # xilinx.com:bd_rule:board down a broken cell-level fallback path (creates a
            # mistyped port and fails connect_bd_net).
            mbv_automation_board_sys_clock [get_bd_intf_pins clk_wiz_1/CLK_IN1_D]
            mbv_automation_board_reset    [mbv_clkwiz_reset_pin clk_wiz_1]
    ...
    ##########################################################################
    # LINUX PROCESSOR PRESET
    ##########################################################################
    } elseif {$preset eq "Linux_Processor"} {
        puts "INFO: Linux_Processor preset enabled"
        ...
        if {$mem_ctrl ne "ddr4"} {
            ...
            # Some 7-series boards (e.g. SP701) do not configure ui_addn_clk_0 in
            # their DDR3 MIG board interface.  When that clock is absent the MBV
            # automation falls back to clk_wiz_1, so the periph crossbar runs on
            # clk_wiz_1/clk_out1 — not ui_addn_clk_0.  Detect this and update
            # mem_int so that all downstream Ethernet peripheral automations use the
            # clock the crossbar actually runs on.
            if {[get_bd_pins -quiet mig_7series_0/ui_addn_clk_0] eq ""} {
                set mem_int {Auto}
            }
            ...
            # Ethernet (RGMII / SGMII / SFP) on 7-series boards
            if {[llength [board::get_board_component_interfaces *rgmii*]] > 0 ||
                [llength [board::get_board_component_interfaces *sfp*]]   > 0} {
                ...
                if {$ethenet_board_interface ne "" || $sfp_board_interface ne ""} {
                    create_bd_cell -type ip -vlnv xilinx.com:ip:axi_ethernet axi_ethernet_0
                    lappend inpt axi_ethernet_0/mac_irq
                    lappend inpt axi_ethernet_0/interrupt

                    set mdio_mdc_if   [board::get_board_component_interfaces *mdio_mdc*]
                    set mdio_mdc_if_1 [lindex [split $mdio_mdc_if { }] 0]
                    set phyrst_if     [board::get_board_component_interfaces *phy_reset_out*]
                    set phyrst_if_1   [lindex [split $phyrst_if { }] 0]
                    set rgmii_if      [board::get_board_component_interfaces rgmii*]
                    set rgmii_if_1    [lindex [split $rgmii_if { }] 0]

                    if {$mdio_mdc_if_1 ne ""} {
                        apply_board_connection -board_interface $mdio_mdc_if_1 -ip_intf axi_ethernet_0/mdio -diagram $design_name
                    }
                    if {$phyrst_if_1 ne ""} {
                        apply_board_connection -board_interface $phyrst_if_1 -ip_intf axi_ethernet_0/phy_rst_n -diagram $design_name
                    }
                    if {$rgmii_if ne ""} {
                        apply_bd_automation -rule xilinx.com:bd_rule:axi_ethernet -config {PHY_TYPE "RGMII" FIFO_DMA "DMA"} [get_bd_cells axi_ethernet_0]
                        apply_board_connection -board_interface $rgmii_if_1 -ip_intf axi_ethernet_0/rgmii -diagram $design_name
                        ...
                        if {$ethenet_board_interface ne "" || $sfp_board_interface ne ""} {
                            mbv_automation_ethernet_mgt_clk [get_bd_intf_pins axi_ethernet_0/mgt_clk]
                        }
                    }
                }

                catch {
                    if {[get_bd_cells axi_ethernet_0_gtxclk] ne ""} {
                        set_property name axi_ethernet_0_refclk [get_bd_cells axi_ethernet_0_gtxclk]
                    }
                }
                catch {
                    if {[get_bd_cells axi_ethernet_0_refclk] ne ""} {
                        ...
                        apply_bd_automation -rule xilinx.com:bd_rule:board -config [format {Clk {%s} Manual_Source {Auto}} $mem_int] [get_bd_pins axi_ethernet_0_refclk/clk_in1]
                    }
                }

                apply_bd_automation -rule xilinx.com:bd_rule:clkrst -config [format {Clk {%s} Freq {100} Ref_Clk0 {None} Ref_Clk1 {None} Ref_Clk2 {None}} $mem_int] [get_bd_pins axi_ethernet_0/axis_clk]
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [format {Clk_master {%s} Clk_slave {%s} Clk_xbar {%s} Master {/microblaze_riscv_0 (Periph)} Slave {/axi_ethernet_0/s_axi} ddr_seg {Auto} intc_ip {/microblaze_riscv_0_axi_periph} master_apm {0}} $mem_int $mem_int $mem_int] [get_bd_intf_pins axi_ethernet_0/s_axi]
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [format {Clk_master {%s} Clk_slave {/mig_7series_0/ui_clk} Clk_xbar {/mig_7series_0/ui_clk (200 MHz)} Master {/axi_ethernet_0_dma/M_AXI_MM2S} Slave {/mig_7series_0/S_AXI} ddr_seg {Auto} intc_ip {/axi_smc} master_apm {0}} $mem_int] [get_bd_intf_pins axi_ethernet_0_dma/M_AXI_MM2S]
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [format {Clk_master {%s} Clk_slave {/mig_7series_0/ui_clk} Clk_xbar {/mig_7series_0/ui_clk (200 MHz)} Master {/axi_ethernet_0_dma/M_AXI_S2MM} Slave {/mig_7series_0/S_AXI} ddr_seg {Auto} intc_ip {/axi_smc} master_apm {0}} $mem_int] [get_bd_intf_pins axi_ethernet_0_dma/M_AXI_S2MM]
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [format {Clk_master {%s} Clk_slave {/mig_7series_0/ui_clk} Clk_xbar {/mig_7series_0/ui_clk (200 MHz)} Master {/axi_ethernet_0_dma/M_AXI_SG} Slave {/mig_7series_0/S_AXI} ddr_seg {Auto} intc_ip {/axi_smc} master_apm {0}} $mem_int] [get_bd_intf_pins axi_ethernet_0_dma/M_AXI_SG]
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config [format {Clk_master {%s} Clk_slave {%s} Clk_xbar {%s} Master {/microblaze_riscv_0 (Periph)} Slave {/axi_ethernet_0_dma/S_AXI_LITE} ddr_seg {Auto} intc_ip {/microblaze_riscv_0_axi_periph} master_apm {0}} $mem_int $mem_int $mem_int] [get_bd_intf_pins axi_ethernet_0_dma/S_AXI_LITE]
            }

        } elseif {$mbv_is_scu200} {
            # SCU200: Linux with LPDDR5
            ...
            if {$lpddrmc_board_interface_1 ne ""} {
                ...
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config {Clk_master {Auto} Clk_slave {Auto} Clk_xbar {Auto} Master {/microblaze_riscv_0 (Cached)} Slave {/lpddrmc_0/S0_AXI} ddr_seg {Auto} intc_ip {New AXI SmartConnect} master_apm {0}} [get_bd_intf_pins lpddrmc_0/S0_AXI]
                apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config {Clk_master {Auto} Clk_slave {Auto} Clk_xbar {Auto} Master {/microblaze_riscv_0 (Cached)} Slave {/lpddrmc_0/S1_AXI} ddr_seg {Auto} intc_ip {New AXI SmartConnect} master_apm {0}} [get_bd_intf_pins lpddrmc_0/S1_AXI]
                if {$mbv_is_lpddrmc_4g} {
                    set_property CONFIG.LPDDR_ADDRESS_BLOCK_BASEADDR 0x100000000 [get_bd_cells /lpddrmc_0]
                } else {
                    set_property CONFIG.LPDDR_ADDRESS_BLOCK_BASEADDR 0x80000000 [get_bd_cells /lpddrmc_0]
                }
                set_property CONFIG.USER_XSDB_INTF_EN TRUE [get_bd_cells lpddrmc_0]
            ...
            if {$usl_ethernet ne ""} {
                ...
                if {$lpddrmc_board_interface_1 ne ""} {
                    ...
                    if {$mbv_is_lpddrmc_4g} {
                        set_property CONFIG.c_addr_width {64} [get_bd_cells axi_ethernet_0_dma]
                    }
                }
                lappend inpt axi_ethernet_0/mac_irq
                lappend inpt axi_ethernet_0/interrupt
            ...
            # SGMII Ethernet with DDR4
            set usl_ethernet [lindex [board::get_board_component_interfaces *sgmii*] 0]
            if {$usl_ethernet ne ""} {
                mbv_connect_sgmii_ethernet_board $design_name $design_name
            ...
            # Fallback Linux: automation with clk_wiz (no DDR board interface)
        ...
        # Linux preset: peripheral connections
        if {$qspi_flash_board_interface ne ""} {
            mbv_automate_peripheral $mem_int /axi_quad_spi_0/AXI_LITE
            if {$mem_ctrl ne "ddr4"} {
                if {$ethenet_board_interface ne ""} {
        ...
        if {$ddr3_board_interface_1 eq "" && $ddr4_board_interface_1 eq "" && !$mbv_is_scu200} {
            create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect axi_smartconnect_0
            create_bd_cell -type ip -vlnv xilinx.com:ip:axi_bram_ctrl axi_bram_ctrl_0
            set_property -dict [list CONFIG.NUM_SI {5} CONFIG.NUM_MI {1}] [get_bd_cells axi_smartconnect_0]
            connect_bd_intf_net [get_bd_intf_pins microblaze_riscv_0/M_AXI_DC] -boundary_type upper [get_bd_intf_pins axi_smartconnect_0/S00_AXI]
            connect_bd_intf_net [get_bd_intf_pins microblaze_riscv_0/M_AXI_IC] -boundary_type upper [get_bd_intf_pins axi_smartconnect_0/S01_AXI]
        ...
        # XDC constraints for Linux preset
        set proj_dir  [get_property DIRECTORY [current_project]]
        set proj_name [get_property NAME      [current_project]]
        file mkdir $proj_dir/$proj_name.srcs/constrs_1/constrs
        set fd [open $proj_dir/$proj_name.srcs/constrs_1/constrs/top.xdc w]
        ...
        if {[get_ips -quiet ${design_name}_axi_ethernet_0_0] ne ""} {
            mbv_write_ethernet_xdc_extras $fd
        }
        if {$mbv_series_7 && $qspi_flash_board_interface ne ""} {
            mbv_write_qspi_7series_xdc $fd
        }
        close $fd
        if {!$mbv_is_scu200} {
            add_files -fileset constrs_1 [list "$proj_dir/$proj_name.srcs/constrs_1/constrs/top.xdc"]
        ...
        puts "ERROR: Invalid preset option: '$preset' (expected Microcontroller, Real-time_Processor, or Linux_Processor)"
    }
    ...
    # SCU200 Linux preset: enable MDM UART and wire its interrupt
    if {$mbv_is_scu200 && $preset eq "Linux_Processor"} {
        set_property CONFIG.C_USE_UART {1} [get_bd_cells mdm_1]
        apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config {Clk_master {/clk_wiz_1/clk_out1 (100 MHz)} Clk_slave {Auto} Clk_xbar {/clk_wiz_1/clk_out1 (100 MHz)} Master {/microblaze_riscv_0 (Periph)} Slave {/mdm_1/S_AXI} ddr_seg {Auto} intc_ip {/microblaze_riscv_0_axi_periph} master_apm {0}} [get_bd_intf_pins mdm_1/S_AXI]
        set_property CONFIG.NUM_PORTS {10} [get_bd_cells microblaze_riscv_0_ilconcat]
        connect_bd_net [get_bd_pins mdm_1/Interrupt] [get_bd_pins microblaze_riscv_0_ilconcat/In9]
    }

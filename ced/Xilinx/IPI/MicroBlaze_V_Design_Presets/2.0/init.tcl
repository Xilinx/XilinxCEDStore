...
proc getSupportedBoards {} {
  ...
  # Parenthesized the DISPLAY_NAME OR-chain so the VENDOR_NAME/NAME conditions apply
  # to every family (Kintex/Artix/Virtex), not just Spartan -- Tcl && binds tighter
  # than ||, so without this the exclusions below would only ever apply to Spartan
  # matches. kcu1500 (Kintex) and vcu1525 (Virtex) are confirmed unsupported by this
  # CED (instantiate_example_design rejects them even though they pass the plain
  # DISPLAY_NAME filter) -- excluded here so the board picker doesn't offer them.
  return [get_board_parts -filter {((DISPLAY_NAME =~"*Kintex*" || DISPLAY_NAME =~"*Artix*" || DISPLAY_NAME =~"*Virtex*" || DISPLAY_NAME =~"*Spartan*") && VENDOR_NAME=="xilinx.com" && (NAME !~ "*kcu1500*") && (NAME !~ "*vcu1525*"))} -latest_file_version -quiet]
}
...
proc addGUILayout {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
 ...
 updater {PROJECT_PARAM.BOARD_PART Preset.VALUE} {Image.IMAGE_PATH Preset.ENABLEMENT Preset.DISPLAYNAME} {
  ...
  if { ${Preset.VALUE} == "Linux_Profile"} {
     set Preset.ENABLEMENT true
     set Image.IMAGE_PATH "microblaze-linux-processor.jpg"

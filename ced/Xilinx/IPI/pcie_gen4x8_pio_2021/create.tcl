...
set files [list \
 [file normalize "${currentDir}/sources/ep_mem.v"] \
 [file normalize "${currentDir}/sources/pcie_app_versal.v"] \
 [file normalize "${currentDir}/sources/pio.v"] \
 [file normalize "${currentDir}/sources/pio_ep.v"] \
 [file normalize "${currentDir}/sources/pio_ep_mem_access.v"] \
 [file normalize "${currentDir}/sources/pio_intr_ctrl.v"] \
 [file normalize "${currentDir}/sources/pio_rx_engine.v"] \
 [file normalize "${currentDir}/sources/pio_to_ctrl.v"] \
 [file normalize "${currentDir}/sources/pio_tx_engine.v"] \
 [file normalize "${currentDir}/sources/design_1_wrapper.v"] \
 [file normalize "${currentDir}/sources/tests.vh"] \
]
...
set file "[file normalize "$currentDir/constraints/top_impl.xdc"]"
set file_added [add_files -norecurse -fileset $obj [list $file]]
set file "$currentDir/constraints/top_impl.xdc"
set file [file normalize $file]
...
set infile [open [file join [get_property directory [current_project]] [current_project].srcs sources_1 imports sources design_1_wrapper.v]]
set contents [read $infile]
close $infile
set contents [string map [list  "design_1 " "$design_name "] $contents]

set outfile  [open [file join [get_property directory [current_project]] [current_project].srcs sources_1 imports sources design_1_wrapper.v] w]
...
set files [list \
 [file normalize "${currentDir}/pre_place.tcl"] \
]
add_files -norecurse -fileset $obj $files

# Set 'utils_1' fileset file properties for remote files
set file "$currentDir/pre_place.tcl"
set file [file normalize $file]

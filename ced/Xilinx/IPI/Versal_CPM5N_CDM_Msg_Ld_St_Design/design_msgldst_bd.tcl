...
variable design_name_msgldst
set design_name_msgldst msgld_fifos

# If you do not already have an existing IP Integrator design open,
# you can create a design using the following command:
#    create_bd_design $design_name_msgldst
...
if { ${design_name_msgldst} eq "" } {
   # USE CASES:
   #    1) Design_name not set

   set errMsg "Please set the variable <design_name_msgldst> to a non-empty value."
   set nRet 1

} elseif { ${cur_design} ne "" && ${list_cells} eq "" } {
   # USE CASES:
   #    2): Current design opened AND is empty AND names same.
   #    3): Current design opened AND is empty AND names diff; design_name_msgldst NOT in project.
   #    4): Current design opened AND is empty AND names diff; design_name_msgldst exists in project.

   if { $cur_design ne $design_name_msgldst } {
      common::send_gid_msg -ssname BD::TCL -id 2001 -severity "INFO" "Changing value of <design_name_msgldst> from <$design_name_msgldst> to <$cur_design> since current design is empty."
      set design_name_msgldst [get_property NAME $cur_design]
   }
   common::send_gid_msg -ssname BD::TCL -id 2002 -severity "INFO" "Constructing design in IPI design <$cur_design>..."

} elseif { ${cur_design} ne "" && $list_cells ne "" && $cur_design eq $design_name_msgldst } {
   # USE CASES:
   #    5) Current design opened AND has components AND same names.

   set errMsg "Design <$design_name_msgldst> already exists in your project, please set the variable <design_name_msgldst> to another value."
   set nRet 1
} elseif { [get_files -quiet ${design_name_msgldst}.bd] ne "" } {
   # USE CASES: 
   #    6) Current opened design, has components, but diff names, design_name_msgldst exists in project.
   #    7) No opened design, design_name_msgldst exists in project.
   ...
   # USE CASES:
   #    8) No opened design, design_name_msgldst not in project.
   #    9) Current opened design, has components, but diff names, design_name_msgldst not in project.

   common::send_gid_msg -ssname BD::TCL -id 2003 -severity "INFO" "Currently there is no design <$design_name_msgldst> in project, so creating one..."

   create_bd_design $design_name_msgldst

   common::send_gid_msg -ssname BD::TCL -id 2004 -severity "INFO" "Making design <$design_name_msgldst> as current_bd_design."
   current_bd_design $design_name_msgldst

}

common::send_gid_msg -ssname BD::TCL -id 2005 -severity "INFO" "Currently the variable <design_name_msgldst> is equal to \"$design_name_msgldst\"."
...
proc create_root_design { parentCell } {
  ...
  # Create ports
  set data_count_0 [ create_bd_port -dir O -from 10 -to 0 data_count_0 ]
  ...
  set wr_data_count_0 [ create_bd_port -dir O -from 10 -to 0 wr_data_count_0 ]
  set wr_data_count_1 [ create_bd_port -dir O -from 10 -to 0 wr_data_count_1 ]

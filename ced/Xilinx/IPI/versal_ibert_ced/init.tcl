...
proc gui_refclk_choice_generation {object_quad source_quads {displayed_quads {}}} {
  ...
  # Separate gui_updater: reset ref VALUE to local refclk when a source quad's line rate
  # exceeds the sharing threshold. Only watches _lr.VALUE (never an output of any updater)
  # so this is DAG-safe. Only resets VALUE when it currently holds a non-local reference
  # (a quad name), so quads that already have a local refclk selected are not touched
  # and won't appear in the serialized options.
  if {[llength $source_quads] > 0} {
    set val_watch_params ""
    set val_change_params ${object_quad}_ref.VALUE
    set val_code ""
    set default_ref [lindex [get_reflocs $object_quad] 0]
    
    foreach quad $source_quads {
      lappend val_watch_params ${quad}_lr.VALUE
      set val_quad_code [list \
        "if \{\[regexp \{X\\d+Y\\d+\} \$\{${object_quad}_ref.VALUE\}\] == 0 && \$\{${quad}_lr.VALUE\} >= $max_ref\} \{" \
        "  set ${object_quad}_ref.VALUE $default_ref" \
        "\}\n" \
      ]
      set val_quad_code [join $val_quad_code "\n"]
      set val_code "$val_code$val_quad_code"
    }
    
    log "gui_updater (value reset) \{$val_watch_params\} \{$val_change_params\} \{\n$val_code\n\}"
    gui_updater $val_watch_params $val_change_params $val_code
  }

...
set ddr_int LPDDR0
set DDRNoc LPDDRNoC0
set LPDDR_int LPDDR1
set LPDDRNoc LPDDRNoC1
} else {
set ddr_int DDR
set DDRNoc DDRNoC
set LPDDR_int LPDDR
set LPDDRNoc LPDDRNoC
}

# Create DDR ports and apply its properties 
create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${ddr_int}_0 -default_portmap
set_property -dict [ list \
	CONFIG.ADDR_WIDTH {64} \
	CONFIG.COMPUTED_STRATEGY {load} \
	CONFIG.INI_STRATEGY {load} \
 ] [get_bd_intf_ports ${ddr_int}_0]
   

set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${ddr_int}_0] 

create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${ddr_int}_1 -default_portmap
set_property -dict [ list \
	CONFIG.ADDR_WIDTH {64} \
	CONFIG.COMPUTED_STRATEGY {load} \
	CONFIG.INI_STRATEGY {load} \
 ] [get_bd_intf_ports ${ddr_int}_1]

set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${ddr_int}_1] 

create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${ddr_int}_2 -default_portmap
set_property -dict [ list \
	CONFIG.ADDR_WIDTH {64} \
	CONFIG.COMPUTED_STRATEGY {load} \
	CONFIG.INI_STRATEGY {load} \
 ] [get_bd_intf_ports ${ddr_int}_2]

set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${ddr_int}_2] 

create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${ddr_int}_3 -default_portmap
set_property -dict [ list \
	CONFIG.ADDR_WIDTH {64} \
	CONFIG.COMPUTED_STRATEGY {load} \
	CONFIG.INI_STRATEGY {load} \
 ] [get_bd_intf_ports ${ddr_int}_3]

set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${ddr_int}_3]

if {[regexp "vck190" $board_name]} {
set_property APERTURES {{0x0 2G} {0x8_0000_0000 6G}} [get_bd_intf_ports ${ddr_int}_0]
set_property APERTURES {{0x0 2G} {0x8_0000_0000 6G}} [get_bd_intf_ports ${ddr_int}_1]
set_property APERTURES {{0x0 2G} {0x8_0000_0000 6G}} [get_bd_intf_ports ${ddr_int}_2]
set_property APERTURES {{0x0 2G} {0x8_0000_0000 6G}} [get_bd_intf_ports ${ddr_int}_3]
} else {
set_property APERTURES {{0x0 2G} {0x8_0000_0000 2G}} [get_bd_intf_ports ${ddr_int}_0]
set_property APERTURES {{0x0 2G} {0x8_0000_0000 2G}} [get_bd_intf_ports ${ddr_int}_1]
set_property APERTURES {{0x0 2G} {0x8_0000_0000 2G}} [get_bd_intf_ports ${ddr_int}_2]
set_property APERTURES {{0x0 2G} {0x8_0000_0000 2G}} [get_bd_intf_ports ${ddr_int}_3]
}
...
set_property -dict [list CONFIG.NUM_CLKS {1} CONFIG.NUM_MI {0} CONFIG.NUM_NMI {4} CONFIG.NUM_SI {4} ] [get_bd_cells ${DDRNoc}]
...
if { $use_lpddr } {
	...
	create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${LPDDR_int}_0 -default_portmap
	set_property -dict [ list \
		CONFIG.ADDR_WIDTH {64} \
		CONFIG.COMPUTED_STRATEGY {load} \
		CONFIG.INI_STRATEGY {load} \
	 ] [get_bd_intf_ports ${LPDDR_int}_0]
	 
	 set_property APERTURES {{0x500_0000_0000 8G}} [get_bd_intf_ports ${LPDDR_int}_0]
	 set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${LPDDR_int}_0] 

	create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${LPDDR_int}_1 -default_portmap
	   
	 set_property -dict [ list \
		CONFIG.ADDR_WIDTH {64} \
		CONFIG.COMPUTED_STRATEGY {load} \
		CONFIG.INI_STRATEGY {load} \
	  ] [get_bd_intf_ports ${LPDDR_int}_1]
	
	set_property APERTURES {{0x500_0000_0000 8G}} [get_bd_intf_ports ${LPDDR_int}_1]
	set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${LPDDR_int}_1] 

	create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${LPDDR_int}_2 -default_portmap
	   
	set_property -dict [ list \
		CONFIG.ADDR_WIDTH {64} \
		CONFIG.COMPUTED_STRATEGY {load} \
		CONFIG.INI_STRATEGY {load} \
	 ] [get_bd_intf_ports ${LPDDR_int}_2]
	
	set_property APERTURES {{0x500_0000_0000 8G}} [get_bd_intf_ports ${LPDDR_int}_2]
	set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${LPDDR_int}_2] 

	create_bd_intf_port -mode Master -vlnv xilinx.com:interface:inimm_rtl:1.0 ${LPDDR_int}_3 -default_portmap
	  
	set_property -dict [ list \
		CONFIG.ADDR_WIDTH {64} \
		CONFIG.COMPUTED_STRATEGY {load} \
		CONFIG.INI_STRATEGY {load} \
	 ] [get_bd_intf_ports ${LPDDR_int}_3]
	 
	set_property APERTURES {{0x500_0000_0000 8G}} [get_bd_intf_ports ${LPDDR_int}_3]
	set_property HDL_ATTRIBUTE.LOCKED {TRUE} [get_bd_intf_ports ${LPDDR_int}_3] 
	  
	# Create instance: LPDDRNoc, and set properties
	
	set ${LPDDRNoc} [ create_bd_cell -type ip -vlnv xilinx.com:ip:axi_noc ${LPDDRNoc} ]
	set_property -dict [list CONFIG.NUM_CLKS {1} CONFIG.NUM_MI {0} CONFIG.NUM_NMI {4} CONFIG.NUM_SI {4} ] [get_bd_cells ${LPDDRNoc}]
	...
	connect_bd_intf_net [get_bd_intf_ports ${LPDDR_int}_0] [get_bd_intf_pins ${LPDDRNoc}/M00_INI]
	connect_bd_intf_net [get_bd_intf_ports ${LPDDR_int}_1] [get_bd_intf_pins ${LPDDRNoc}/M01_INI]
	connect_bd_intf_net [get_bd_intf_ports ${LPDDR_int}_2] [get_bd_intf_pins ${LPDDRNoc}/M02_INI]
	connect_bd_intf_net [get_bd_intf_ports ${LPDDR_int}_3] [get_bd_intf_pins ${LPDDRNoc}/M03_INI]
...
if { $use_aie } {
	...
	#set_property APERTURES { {{0x200_0000_0000 4G}}} [get_bd_intf_ports AIE_CTRL_INI]
...
# Clock options, and set properties
...
set mbufgce_enab_boards [ list vck190 vck190_newl vek280 ]
...
foreach { port freq id is_default } $clk_options {
	...
	if { [lsearch $mbufgce_enab_boards $board_name] != -1 } {
	...
	puts "INFO: Clock port: $port, Frequency: $freq, ID: $id, Default: $is_default"
...
if { $use_cascaded_irqs } {
	...
	create_bd_cell -type inline_hdl -vlnv xilinx.com:inline_hdl:ilconcat:* xlconcat_2
...
for {set i 0} {$i < $num_kernal} {incr i} {
	...
	connect_bd_net [get_bd_pins proc_sys_reset_${default_clk_num}/peripheral_aresetn] [get_bd_pins to_delete_kernel_ctrl_${i}/aresetn]
	connect_bd_net -net proc_sys_reset_${default_clk_num}_peripheral_aresetn [get_bd_pins icn_ctrl_$m/aresetn]

	connect_bd_net [get_bd_pins clk_wizard_0/$default_clk_port] [get_bd_pins icn_ctrl_$m/aclk]

	connect_bd_net [get_bd_pins to_delete_kernel_ctrl_${i}/aclk] [get_bd_pins clk_wizard_0/$default_clk_port]
	
} 


# Connect clk_wizard clock_out ports to proc_sys_reset slowest_sync_clk ports
set u 0
set p_id 0
if { $mbufgce } {
		...
		if {$clk_u == "true"} {
			...
			if { $port == $default_mbuf_port} {
				...
				for {set k 1} {$k < 5} {incr k} {
					#puts "***** ${mbuf_cklport}$k *****"
					connect_bd_net [get_bd_pins clk_wizard_0/${mbuf_cklport}$k] [get_bd_pins proc_sys_reset_[expr $u+$l]/slowest_sync_clk]
					incr l
...
connect_bd_intf_net [get_bd_intf_ports ${ddr_int}_3] [get_bd_intf_pins ${DDRNoc}/M03_INI]
connect_bd_intf_net [get_bd_intf_ports ${ddr_int}_0] [get_bd_intf_pins ${DDRNoc}/M00_INI]
connect_bd_intf_net [get_bd_intf_ports ${ddr_int}_1] [get_bd_intf_pins ${DDRNoc}/M01_INI]
connect_bd_intf_net [get_bd_intf_ports ${ddr_int}_2] [get_bd_intf_pins ${DDRNoc}/M02_INI]
connect_bd_net [get_bd_ports ExtClk] [get_bd_pins ${DDRNoc}/aclk0]
connect_bd_intf_net [get_bd_intf_pins IsoRegDynamic/M_AXI] [get_bd_intf_pins icn_ctrl_1/S00_AXI]
connect_bd_intf_net [get_bd_intf_ports PL_CTRL_S_AXI] [get_bd_intf_pins IsoRegDynamic/S_AXI]

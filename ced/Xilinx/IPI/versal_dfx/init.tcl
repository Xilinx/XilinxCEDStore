...
proc getSupportedBoards {} {
	return [get_board_parts -filter {(BOARD_NAME =~"*vck190*" && VENDOR_NAME=="xilinx.com")||(BOARD_NAME =~"*vek280*" && VENDOR_NAME=="xilinx.com")} -latest_file_version -quiet]
}
...
proc addOptions {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
    ...
    lappend x [dict create name "Clock_Options" type "string" value "clk_out1 625 0 true clk_out2 250 1 false clk_out3 104.167 2 false" enabled true]
...
proc addGUILayout {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
	...
	during generation. User should review the \"Messages\" window once the design is created to ensure that the selected clock frequencies are correctly derived. 
	The specified Default Clock will drive all IPs created by this wizard. Choosing higher clock frequencies may pose challenges during Timing Closure. 
	On selecting 625MHz for the default clock, MBUFGCE is enabled in clocking wizard to generate 4 divided clock ports and the second divided clock is used for driving the design created by this wizard."
...
validater { Clock_Options.VALUE } { Clock_Options.ERRMSG } {
    ...
    foreach { port } [ lsort -unique $clk_ports ] {
        ...
        # if {[regexp $port [ lsort -unique $clk_ports ]] } {
        #     set Clock_Options.INFO "Found a clock name contained within another clock name. This could lead to incorrect clock assignment and errors. Please set unique name for each clock port."
        #     set text "Found a clock name contained within another clock name. This could lead to incorrect clock assignment and errors. Please set unique name for each clock port."
        #     ced::add_text -designObject $designObj -name Note -tclproc $text  -parent $clocks
        # }

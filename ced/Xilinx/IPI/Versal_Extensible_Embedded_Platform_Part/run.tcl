...
set clk_options { clk_out1 156.250000 0 true clk_out2 104.166666 1 false clk_out3 312.500000 2 false }
...
create_root_design $currentDir $design_name $use_lpddr $clk_options $irqs $use_aie
	...
	if { $use_aie eq "true" } {
		...
		\t --> AI Engine control path is connected to CIPS. V++ will connect AI Engine data path automatically} [current_bd_design]
	
	} else {
		
		set_property USER_COMMENTS.comment0 {\t \t \t =============== >>>> An Example Versal Extensible Embedded Platform <<<< ===============
		\t Note: 
		\t --> SD boot mode and UART are enabled in the CIPS} [current_bd_design]
	}
	...
	if { $use_aie == "true" } {
	   ...
	   regenerate_bd_layout -layout_string {
		   ...
		   "comment_0":"\t \t \t =============== >>>> An Example Versal Extensible Embedded Platform <<<< ===============
			\t Note: 
			\t --> SD boot mode and UART are enabled in the CIPS
			\t --> AI Engine control path is connected to CIPS. V++ will connect AI Engine data path automatically",
		   "commentid":"comment_0|",
		   "font_comment_0":"14",

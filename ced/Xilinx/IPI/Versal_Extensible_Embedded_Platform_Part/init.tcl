...
proc getSupportedParts {} {
	...
	# Filtered out unsupported parts
		if {![regexp "1" [[regexp "xcvp1402-vsvd2197" $item]||[regexp "xcvp1902" $item]||[regexp "xcvn3716" $item]||[regexp "xc2ve3558-sbva1440" $item]||[regexp "xc2vm3558-sbva1440" $item]||[regexp "xc2ve3504-sbva1440" $item]]]} {
...
gui_updater {PROJECT_PARAM.PART} {Include_AIE.VISIBLE Include_AIE.ENABLEMENT Include_AIE.VALUE Include_BDC.ENABLEMENT} {
	set gui_flag 0
	...
	foreach get_aie_prop $V_Part {
		...
		if {([regexp "AIE_ENGINE" [lindex $get_aie_prop 1 ]] == 1) && ([lindex $get_aie_prop 3 ] != 0) } {
			...
			set Include_AIE.ENABLEMENT true
			if {[regexp "xcvm2152" ${PROJECT_PARAM.PART}] || [regexp "xc2v" ${PROJECT_PARAM.PART}] || [regexp "xcvr1652" ${PROJECT_PARAM.PART}] || [regexp "xcvr1602" ${PROJECT_PARAM.PART}]} {

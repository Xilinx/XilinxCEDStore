...
proc getConfigDesignInfo {} {
  return [dict create name {ZYNQUS+MPSOC} description {ZYNQ+MPSOC system with peripherals including UART and DDR4}]
}
...
proc getSupportedBoards {} {
    #return [get_board_parts -filter {(PART_NAME =~"*xc*u*" && PART_NAME !~"*xczu*") || (PART_NAME =~"*XC*U*" && PART_NAME !~"*XCZU*") && BOARD_NAME!~*adm-pcie-7v3* } -latest_file_version]
#return [get_board_parts -filter {(PART_NAME =~"*xczu*") || (PART_NAME =~"*XC*U*") && BOARD_NAME!~*adm-pcie-7v3* } -latest_file_version]	
#return [get_board_parts -filter {(PART_NAME =~"*xczu9eg*" && VENDOR_NAME=="xilinx.com" ) || (PART_NAME =~"*xczu7ev*" && VENDOR_NAME=="xilinx.com")}  -latest_file_version]	
...
proc addOptions {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
   ...
   lappend x [dict create name "Include_UART_BAUD" type "string" value "115200" value_list { 4800 9600 19200 38400 57600 115200 128000 230400 } enabled true]
...
proc addGUILayout {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
  ...
  set combinedPanel [ced::add_panel -name combinedPanel -parent $panel3 -designObject $designObj] 
  ...
  set gpio_panel [ced::add_panel -name gpio_panel -parent $page -designObject $designObj -layout horizontal]
	#set GPIOPanel [ced::add_panel -name combinedPanel -parent $page -designObject $designObj] 
	set gpioGroup [ced::add_group -name "General Purpose IOs" -display_name "General Purpose IOs"  -parent $gpio_panel -designObject $designObj ]	
  set gpio_switch_if [get_property DISPLAY_NAME [get_board_components -filter {SUB_TYPE==switch || SUB_TYPE==led || SUB_TYPE==push_button } -of_objects [get_boards [regsub "(part.*:)" ${PROJECT_PARAM.BOARD_PART} {}]]]]
  foreach item $gpio_switch_if {
    ced::add_param -name Include_[regsub " " $item {_}]  -display_name "$item" -parent $gpioGroup -designObject $designObj
  }
 
  set group4 [ced::add_group -name "UART_Options" -display_name "UART Options"  -parent $gpio_panel -designObject $designObj -visible true -header_param "Include_UART"]
...
if {${PROJECT_PARAM.BOARD_PART} != "" } {
 ...
 # gui_updater {PROJECT_PARAM.BOARD_PART} {Include_IIC0.VISIBLE} {
 # if {${PROJECT_PARAM.BOARD_PART} != "" } {
  # if { [get_board_components -filter {SUB_TYPE==mux} -of_objects [get_boards [regsub "(part.*:)" ${PROJECT_PARAM.BOARD_PART} {}]]] != ""} {
    # if { [lindex [get_property SUB_TYPE [get_board_components -filter {SUB_TYPE==uart} -of_objects [get_boards [regsub "(part.*:)" ${PROJECT_PARAM.BOARD_PART} {}]]]] 0] == "mux" } {
       # set Include_IIC0.VISIBLE true
     # } else {
       # set Include_IIC0.VISIBLE false
           # }
     # } else {
     # set Include_IIC0.VISIBLE false
    # }

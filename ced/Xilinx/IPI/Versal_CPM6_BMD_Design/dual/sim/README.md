# Versal CPM6 BMD Example Design (Dual Controller)

| Item | Summary |
|---|---|
| **Primary Purpose** | Demonstrates PCIe Gen6 Bus Master DMA (BMD) capability of the Versal CPM6 hard IP, using both Controller 0 and Controller 1 as independent Endpoints |
| **Example Type** | IP Example Design |
| **Target Audience** | Verification/FPGA engineers validating the CPM6 PCIe hard IP |
| **Devices Supported** | Versal devices with CPM6 hard IP (`vsvc3340` package family) |
...
| **Not Intended For** | Production deployment, performance benchmarking, hardware bring-up |
| **Time to First Success** | ~20-25 minutes (compile + optimize + simulate, order of magnitude) |

---

## Overview
This example design demonstrates Bus Master DMA (BMD) capability of the Versal CPM6 hard IP in a dual-controller PCIe Endpoint simulation configuration -- both Controller 0 and Controller 1 are active as independent Endpoints.

By working through this example design, you will learn how to:
- Configure CPM6 lane rate (16/32/64 GT/s) and link width (X1/X2/X4/X8) for both Controller 0 and Controller 1
- Drive PCIe Write/Read DMA traffic against both CPM6 controllers through a UVM testbench
...
- **Error injection** -- bad data injection, Unsupported Request (UR) generation

### Architecture Overview
```
tb_top
 └─ ctrl_inst
     └─ dut_inst (dual_ctrl_bmd_ep)
         ├─ design_1_wrapper           -- Vivado-generated BD wrapper; contains the
         │                                encrypted CPM6 hard IP (GT quads, PCIe hard
         │                                block, both controllers) -- not visible in
         │                                waveform/source view
         └─ pcie_app_versal_bmd_top_i  -- open user logic (BMD_AXIST_*): DMA engine,
                                          CSR register file, interrupt control, TPH/VSEC
                                          (instantiated per controller)
```
A UVM testbench (Avery PCIe VIP as Root Complex, one instance per controller) drives configuration and traffic against the DUT and checks responses via scoreboards.
...
3. Via the GUI: File → Project → New (or IP Catalog → Example Designs) and select "Versal CPM6 BMD Design".
   ...
   Alternately, users can use command line options in the Vivado Tcl console:
   ```
...
**Caution:**
Changes to the scoreboards (`test/scoreboards/`) or capability sequences (`test/sequences/capability/`) affect correctness checking across all tests -- verify carefully before relying on results from a modified scoreboard.

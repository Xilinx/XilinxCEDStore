class gpmon_txn extends base_txn;

  `uvm_object_utils(gpmon_txn)
  ...
  function new(string name = "gpmon_txn");
    super.new(name);
    txn_type = "GPMON_TXN";
  endfunction 
  ...
  virtual function void do_copy(uvm_object rhs);
    ...
    super.do_copy(rhs);
    if (!$cast(t,rhs))
      `uvm_fatal(get_type_name, "do_copy got a txn that wasn't extended from gpmon_txn")
    width       = t.width;
    sig         = t.sig;
    sig_enum    = t.sig_enum;
  ...
  virtual function bit do_compare(uvm_object rhs, uvm_comparer comparer);
    ...
    if (!$cast(t, rhs))
      `uvm_fatal(get_type_name, "do_compare got a txn that wasn't extended from gpmon_txn")

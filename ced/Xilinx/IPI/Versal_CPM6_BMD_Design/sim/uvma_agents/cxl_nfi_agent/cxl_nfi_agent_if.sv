...
interface cxl_nfi_agent_if #(
  ...
  // Equivalent to $past(ready) for ready_q
  logic ready_q;
  initial ready_q <= 1'b0;
  always @(posedge clk)
    ready_q <= ready;
  ...
  property valid_only_ready_check;
    @(posedge clk iff (valid && valid_only_mode))
    disable iff (!en_assert)
    ready || ready_q;
  endproperty
  ...
  aprop_vom_ready  : assert property(valid_only_ready_check) else
                       `uvm_error("cxl_nfi_agent_if", "aprop_vom_ready assertion failure")
  // Per-flit checks
  generate 

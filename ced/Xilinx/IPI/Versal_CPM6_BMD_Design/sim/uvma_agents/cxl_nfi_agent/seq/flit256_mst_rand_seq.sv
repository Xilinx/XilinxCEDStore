      ...
      while (sptr<15) begin
        ...
        if (!sptr || !dat_q.size) begin
          ...
          if (dat_q[0].txn_type == "H2D_DAT") begin
            sptr_str.itoa(sptr);
            `uvm_info(get_type_name, $sformatf("sptr=%2s -> DATA (%0d)",sptr_str,dd), UVM_NONE)
            $cast(h2ddat, dat_q[0]);
            ...
            if (dd == 4) begin
              sptr_str.itoa(sptr);
              `uvm_info(get_type_name, $sformatf("sptr=%2s -> TRAILER",sptr_str), UVM_NONE)
        ...
        if (did_repack) begin
          ...
          sptr_str.itoa(sptr);
          `uvm_info(get_type_name, $sformatf("sptr=%2s -> NONE",sptr_str), UVM_NONE)
        end
          ...
          `uvm_info(get_type_name, $sformatf("sptr=%2s, slotfmt=%3s, txntype=%0s, txn#=%0d",
                                              sptr_str, msg, r_tt.name, r_nt), UVM_NONE)
        end
        // go to next slot
        sptr++;
      end

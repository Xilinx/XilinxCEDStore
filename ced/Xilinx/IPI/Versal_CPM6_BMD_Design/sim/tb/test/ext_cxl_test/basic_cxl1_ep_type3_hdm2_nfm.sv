 ...
 * and the HDM to the smallest size (256 MB). Exercises CPM6 Controller 1.
 * EXTENDS
 * base_cxl_ep_type3_hdm2_nfm 
 * ACRONYMS
 * hdm = "host managed device memory"
 * nfm = "non-flit mode"
|*/

class basic_cxl1_ep_type3_hdm2_nfm extends base_cxl_ep_type3_hdm2_nfm;

  `uvm_component_utils(basic_cxl1_ep_type3_hdm2_nfm)
  ...
  virtual function void connect_phase(uvm_phase phase);
    ...
    env.cxl_nfi_agnt_rx[1].base_tl_ap.connect(cxl_mem_rsp.fifo.analysis_export);
    cxl_mem_rsp.api68  = env.cxl_nfi_agnt_tx[1].api68;
    cxl_mem_rsp.api256 = env.cxl_nfi_agnt_tx[1].api256;
  endfunction
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
    ...
    {vip_cfg.ctrlr_en[0], dut_cfg.ctrlr_en[0]} = 2'b0; //Ctrlr0 disabled
    // VIP as RP
    // ---
    // DUT as EP
    //  - Functions
    dut_cfg.pcie_cfg[1].num_pfs = 1;
    dut_cfg.pcie_cfg[1].num_vfs = 0;
    //  - HDM Ranges
    dut_cfg.cxl_cfg[1].dvsec_cxl_cap.hdm_range[1].hi = '0;
    dut_cfg.cxl_cfg[1].dvsec_cxl_cap.hdm_range[1].lo = 1'b1; //256 MB
    dut_cfg.cxl_cfg[1].dvsec_cxl_cap.hdm_range[2].hi = '0;
    dut_cfg.cxl_cfg[1].dvsec_cxl_cap.hdm_range[2].lo = 1'b1; //256 MB
  endfunction
  ...
  virtual task pre_reset_phase(uvm_phase phase);
    ...
    cxl_mbox[1].load_mmio_mbox(0, 0, cxl_dev_reg_if_base, mbox, 'h3_FFFF);
    // Create mailbox capabilities for DSR
    mbox = new[2];
    // -> DSR+0x0 : Event Status Register 
    // -- all zeroes, so nothing to assign
    // Provide the virtual FW mailbox sequence to what we just built
    cxl_mbox[1].load_mmio_mbox(0, 0, cxl_dev_sts_reg_base, mbox, 'h3_FFFF);
    ...
    // -- all zeroes, so nothing to assign
    // Provide the virtual FW mailbox sequence to what we just built
    cxl_mbox[1].load_mmio_mbox(0, 0, cxl_prim_mbox_base, mbox, 'h3_FFFF);
    ...
    // Provide the virtual FW mailbox sequence to what we just built
    cxl_mbox[1].load_mmio_mbox(0, 0, cxl_mem_dev_sts_reg_base, mbox, 'h3_FFFF);
     ...
     */
    cxl_mbox[1].cxl_mbox_pf        = 0;
    cxl_mbox[1].cxl_mbox_bar       = 0;
    cxl_mbox[1].cxl_mbox_base_addr = cxl_prim_mbox_base;
    // Add the command objects 
    // - Default set
    cxl_mbox[1].add_cmd_obj(cmd_0100h);
    cxl_mbox[1].add_cmd_obj(cmd_0101h);
    cxl_mbox[1].add_cmd_obj(cmd_0102h);
    cxl_mbox[1].add_cmd_obj(cmd_0103h);
    cxl_mbox[1].add_cmd_obj(cmd_0300h);
    cxl_mbox[1].add_cmd_obj(cmd_0301h);
    cxl_mbox[1].add_cmd_obj(cmd_0400h);
    cxl_mbox[1].add_cmd_obj(cmd_0401h);
    // - Modified set
    cmd_4000h.opayload.vol_only_cap = 1; //multiple of 256MB
    cmd_4000h.opayload.total_cap    = 1; //multiple of 256MB
    cxl_mbox[1].add_cmd_obj(cmd_4000h);
  endtask
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    // Configure and send a full cacheline CXL.mem write and read to HDM1
    hdm_base = ep_dev.cxl_hdm[1].base;
    hdm_size = ep_dev.cxl_hdm[1].sz;
    hdm_end  = hdm_base+hdm_size;
    ...
    // Configure and send a partial cacheline CXL.mem write and read to HDM1
    cxltlp = amd_cxlmem_tlp::type_id::create("cxltlp");
    void'(this.randomize with { hdm_addr inside {[hdm_base:hdm_end]}; });
    // - WR 
    foreach (wdat[ii]) wdat[ii] = 'h40+ii;

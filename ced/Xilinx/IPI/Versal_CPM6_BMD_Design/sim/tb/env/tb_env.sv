class tb_env extends uvm_env;
  ...
  typedef gpdrv_monitor#(gpdrv_cfg, virtual gpdrv_if, gpdrv_txn) gpdrv_mon_t;
...
`endif
  ...
  virtual gpdrv_if            vif_cxl_pm_in [0:1];
...
`endif
  ...
  gpdrv_agent        cxl_pm_in_agnt [0:1];
...
`endif
  ...
  cxl_pm_in_cb  gpdrv_cb_cxl_pm_in[0:1];
  cxl_pm_out_cb gpmon_cb_cxl_pm_out[0:1];
  isr_cb        gpmon_cb_isr;
...
`endif
    ...
    for (int ii=0; ii<2; ii++) begin
      ...
      if (cfg.cxl_pm_in_agnt[ii] != UNUSED_AGNT)
        if (!uvm_config_db#(virtual gpdrv_if)::get(this, "", $sformatf("vif_cxl_pm_in[%0d]",ii), vif_cxl_pm_in[ii]))
          `uvm_fatal("CFGDB_NOGET", $sformatf("Could not get 'vif_cxl_pm_in[%0d]' from cfg db",ii))
...
`endif
    ...
    for (int ii=0; ii<2; ii++) begin
      ...
      // --- //
      if (cfg.cxl_pm_in_agnt[ii] != UNUSED_AGNT) begin
        cxl_pm_in_agnt[ii] = gpdrv_agent_creator#()::spawn($sformatf("cxl_pm_in_agnt[%0d]",ii),
                                                            this,
                                                            vif_cxl_pm_in[ii]);
        cxl_pm_in_agnt[ii].cfg.width    = 34;
        cxl_pm_in_agnt[ii].cfg.uid      = $sformatf("CXL PM In[%0d]",ii);
        cxl_pm_in_agnt[ii].cfg.activity = cfg.cxl_pm_in_agnt[ii]==ACTIVE_AGNT ? UVM_ACTIVE : UVM_PASSIVE;
      end
      // --- //
...
`endif
    ...
    for (int ii=0; ii<2; ii++) begin
      if (cfg.cxl_pm_out_agnt[ii] != UNUSED_AGNT)
        gpmon_cb_cxl_pm_out[ii] = cxl_pm_out_cb::type_id::create($sformatf("gpmon_cb_cxl_pm_out[%0d]",ii));
    end
    // callbacks for gpdrv
    for (int ii=0; ii<2; ii++) begin
      if (cfg.cxl_pm_in_agnt[ii] != UNUSED_AGNT)
        gpdrv_cb_cxl_pm_in[ii]  = cxl_pm_in_cb ::type_id::create($sformatf("gpdrv_cb_cxl_pm_in[%0d]",ii));
    end
    if (isr_used)
      gpmon_cb_isr = isr_cb::type_id::create("gpmon_cb_isr");
    // scoreboards
...
`endif
    ...
    for (int ii=0; ii<2; ii++) begin
      ...
      if (cfg.cxl_pm_in_agnt[ii]!=UNUSED_AGNT)
        uvm_callbacks#(gpdrv_mon_t, cxl_pm_in_cb) ::add(cxl_pm_in_agnt[ii].mon,  gpdrv_cb_cxl_pm_in[ii]);
      if (cfg.cxl_pm_out_agnt[ii]!=UNUSED_AGNT)
        uvm_callbacks#(gpmon_mon_t, cxl_pm_out_cb)::add(cxl_pm_out_agnt[ii].mon, gpmon_cb_cxl_pm_out[ii]);
      ...
      if (cfg.cxl_pm_in_agnt[ii]!=UNUSED_AGNT)
        cxl_pm_in_agnt[ii]    .base_ap   .connect(base_printer.analysis_export);
      if (cfg.cxl_pm_out_agnt[ii]!=UNUSED_AGNT)
...
`endif
  ...
  virtual function void setup_pl_vip(dut_config cfg);
    foreach (cfg.ctrlr_en[ii]) begin
      ...
      if (this.cfg.cxl_pm_in_agnt[ii] != UNUSED_AGNT) begin
        if (cfg.pcie_cfg[ii].pcie_cap.pcie_cap.dev_port_type==pcie_config::RP) begin
          gpdrv_cb_cxl_pm_in[ii].dut_cxl_host = 1;
          cxl_pm_in_agnt[ii].cfg.addl_txn_info.push_back("Host Mode");
        end
        else begin
          gpdrv_cb_cxl_pm_in[ii].dut_cxl_host = 0;
          cxl_pm_in_agnt[ii].cfg.addl_txn_info.push_back("Device Mode");
        end
      end
      if (this.cfg.cxl_pm_out_agnt[ii] != UNUSED_AGNT) begin
        if (cfg.pcie_cfg[ii].pcie_cap.pcie_cap.dev_port_type==pcie_config::RP) begin
          gpmon_cb_cxl_pm_out[ii].dut_cxl_host = 1;
          cxl_pm_out_agnt[ii].cfg.addl_txn_info.push_back("Host Mode");
        end
        else begin
          gpmon_cb_cxl_pm_out[ii].dut_cxl_host = 0;
          cxl_pm_out_agnt[ii].cfg.addl_txn_info.push_back("Device Mode");
        end
      end

...
();
  ...
  // Required before any 32-bit R5_API access routed through a given port
  // (e.g. M_AXI_LPD): the PS VIP's internal BFM defaults to a 128-bit data
  // width, and without this call a 32-bit write_data_32/read_data_32 can
  // land on the wrong byte lane. Passthrough pattern matches every other
  // task in this file.
  task select_pl_datawidth(bit [1:0] width, string port_name);
    inst.select_pl_datawidth(width, port_name);
  endtask

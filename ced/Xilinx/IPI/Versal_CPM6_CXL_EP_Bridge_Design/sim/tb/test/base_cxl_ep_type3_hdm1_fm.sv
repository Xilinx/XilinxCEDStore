...
class base_cxl_ep_type3_hdm1_fm extends base_cxl_ep_test;
  ...
  // DOE mailbox responder (CR-1268755 / DOE_MB_FIX)
  cseq_cpm6_pcie_core_isr_src     doe_core_isr[0:1];
  cseq_core_doe_discovery_cfg_mb  doe_isr[0:1];
...
`endif
    ...
    // DOE_MB_FIX: two-level hierarchy (register_isr handles routing)
    doe_core_isr[0] = cseq_cpm6_pcie_core_isr_src::type_id::create("doe_core_isr[0]");
    doe_core_isr[0].set_ctrlr(0); doe_core_isr[0].reg_trigger = CORR;
    doe_core_isr[0].bit_trigger = 'h6 << (0*6);
    env.ps_isr_seq.c_seq.push_back(doe_core_isr[0]);
    doe_isr[0] = cseq_core_doe_discovery_cfg_mb::type_id::create("doe_isr[0]");
    doe_isr[0].set_ctrlr(0); doe_core_isr[0].c_seq.push_back(doe_isr[0]);
    doe_core_isr[1] = cseq_cpm6_pcie_core_isr_src::type_id::create("doe_core_isr[1]");
    doe_core_isr[1].set_ctrlr(1); doe_core_isr[1].reg_trigger = CORR;
    doe_core_isr[1].bit_trigger = 'h6 << (1*6);
    env.ps_isr_seq.c_seq.push_back(doe_core_isr[1]);
    doe_isr[1] = cseq_core_doe_discovery_cfg_mb::type_id::create("doe_isr[1]");
    doe_isr[1].set_ctrlr(1); doe_core_isr[1].c_seq.push_back(doe_isr[1]);

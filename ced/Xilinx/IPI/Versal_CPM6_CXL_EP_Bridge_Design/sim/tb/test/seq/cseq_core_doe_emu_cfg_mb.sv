class cseq_core_doe_emu_cfg_mb extends cseq_core_doe_discovery_cfg_mb;
  ...
  // Connects to an external DOE emulator process (see socket_dpi_pkg.sv)
  // listening on this host:port. Point this at wherever your site runs
  // that emulator - this is an example test (test_ide_tlps_spdm.sv), not
  // part of the default cxl_ep_brdg_sanity test flow.
  function new(string name = "cseq_core_doe_emu_cfg_mb");
    super.new(name);

    sock_client = socket_client_create("localhost", 2323);

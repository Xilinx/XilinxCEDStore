...
`endif
  ...
  // uvm_task_phase forcibly kills any remaining fork-join_none processes
  // when the phase completes, so the heartbeat below is started here
  // (phase_started) instead, to survive past pre_reset.
  ...
  virtual function void start_of_simulation_phase(uvm_phase phase);
    ...
    // NOTE: bare "factory" is not a real global in this UVM-1.2 install
    // (unlike e.g. uvm_top, which really is one) - grep across this
    // testbench shows the correct, already-used-elsewhere idiom is a
    // local uvm_factory handle from uvm_factory::get() (see
    // tb/test/examples/test_ide_tlps_spdm.sv). Referencing a bare,
    // undeclared "factory" here compiled fine at vlogan time (VCS defers
    // resolving such an unresolvable local name to a possible upward
    // hierarchical/cross-module reference until elaboration) but failed
    // elaboration with Error-[XMRE] "token 'factory'" - only surfaced now
    // that elaboration was actually attempted for the first time.
    begin

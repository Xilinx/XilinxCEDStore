...
class seq_cxl_cachemem_reqrwd_avery_rand extends seq_cxl_base;
  ...
  // NOTE: this class had no constructor of its own. Per IEEE 1800, a class
  // that doesn't declare its own `new()` gets an IMPLICIT no-argument
  // constructor - it does NOT inherit the parent's (seq_cxl_base's)
  // 1-argument `function new(string name = get_type_name())`. The
  // `uvm_object_utils factory registration macro instantiates
  // uvm_object_registry#(T,...)::create(), which calls `T::new(name)` -
  // with the implicit 0-arg constructor that's "too many arguments",
  // caught only at elaboration (parameterized registry template
  // instantiation is deferred past vlogan) with Error-[TMAFTC]. Every
  // other sequence class in this directory (seq_cap_traverse,
  // seq_ep_get_bars, vseq_base, etc.) already declares this exact
  // pattern - this one was simply missing it.
  function new(string name = "seq_cxl_cachemem_reqrwd_avery_rand");
    super.new(name);
  endfunction
    ...
    foreach(txn_q[i]) begin
      ...
      case(1)
        ...
        $cast(m2sreq, txn_q[i]) : begin
          ...
          // Avery CXL Enumeration: apciexactor 2025.1_cxl acxl_enum.svh (definitions up through 3.2)
        ...
        $cast(m2srwd, txn_q[i]) : begin
          ...
          // Avery CXL Enumeration: apciexactor 2025.1_cxl acxl_enum.svh

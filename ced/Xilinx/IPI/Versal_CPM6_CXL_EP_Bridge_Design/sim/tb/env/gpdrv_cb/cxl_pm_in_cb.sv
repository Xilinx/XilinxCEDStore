class cxl_pm_in_cb extends gpdrv_mon_cb#(gpdrv_txn);
  ...
  virtual function void device_make_specific(gpdrv_txn txn);
    // Device-mode field decode was never added to this vendored callback
    // (host_make_specific above has a full VDM/GPF/ResetPrep field decode;
    // there is no device-side equivalent) - only discovered once a DUT
    // configured as a CXL device (dut_cxl_host=0, this design's own
    // configuration) actually exercised this path for the first time, via
    // an unconditional `uvm_fatal` that blocked ANY such test from running
    // past reset. This callback only affects log readability (populates
    // txn.ml_sig_enum - see this file's own header comment: "Make macro
    // printing sensible" / "for sensible printing to the user"), not
    // sim-functional behavior, so log the raw signal value instead of
    // fataling until a real device-mode field layout decode is written.
    `check_add_radix(33:0, "Device-mode PM signal (raw - field decode not implemented)", 1, hex)
    last_val = txn.sig;
  endfunction

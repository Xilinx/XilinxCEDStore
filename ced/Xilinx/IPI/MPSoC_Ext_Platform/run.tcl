...
proc createDesign {design_name options} {  
  ...
  proc create_root_design {currentDir design_name use_ddr clk_options irqs} {
    ...
    set axi_smartconnect_lpd [ create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect axi_smartconnect_lpd ]
    set_property -dict [ list CONFIG.NUM_SI {1} CONFIG.NUM_MI {1} ] $axi_smartconnect_lpd
    ...
    # Create axi_vip_pltf and set properties
    set axi_vip_pltf [create_bd_cell -type ip -vlnv xilinx.com:ip:axi_vip axi_vip_pltf]
    ...
    set smartconnect_axifull [ create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect smartconnect_axifull ]
    set_property -dict [ list CONFIG.NUM_SI {1} CONFIG.NUM_MI {1} ] $smartconnect_axifull
    ...
    if {!$use_ddr } { 
      ...
      set smartconnect_axihpm0fpd [ create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect smartconnect_axihpm0fpd ]
      set_property -dict [ list CONFIG.NUM_SI {1} CONFIG.NUM_MI {1} ] $smartconnect_axihpm0fpd
    ...
    if {$irqs == "32"} {
      ...
      # set smartconnect_axilite [ create_bd_cell -type ip -vlnv xilinx.com:ip:axi_smartconnect smartconnect_axilite ]
      set smartconnect_axilite [ create_bd_cell -type ip -vlnv xilinx.com:ip:smartconnect smartconnect_axilite ]
      set_property -dict [ list CONFIG.NUM_SI {1} CONFIG.NUM_MI {1} ] $smartconnect_axilite
    ...
    if {$use_ddr } {
      ...
      if {$ddr4_board_interface_1 == ""} {
        puts "$board_name does not support pl_ddr!"}
      ...
      connect_bd_intf_net [get_bd_intf_pins axi_vip_pltf/S_AXI] [get_bd_intf_pins axi_smc/M01_AXI]
    ...
    if {$irqs == "32"} {
      ...
      connect_bd_net [get_bd_pins clk_wiz_0/$default_clk_port] [get_bd_pins smartconnect_axilite/aclk] [get_bd_pins axi_intc_0/s_axi_aclk]

      connect_bd_net [get_bd_pins proc_sys_reset_${default_clk_num}/interconnect_aresetn] [get_bd_pins smartconnect_axilite/aresetn] [get_bd_pins axi_intc_0/s_axi_aresetn] 
    }
    ...
    connect_bd_net [get_bd_pins clk_wiz_0/$default_clk_port]  [get_bd_pins axi_smartconnect_lpd/aclk] [get_bd_pins axi_vip_pltf/aclk] [get_bd_pins axi_vip_0/aclk] [get_bd_pins axi_vip_1/aclk] [get_bd_pins smartconnect_axifull/aclk] [get_bd_pins ps_e/maxihpm0_fpd_aclk] [get_bd_pins ps_e/maxihpm0_lpd_aclk] [get_bd_pins ps_e/saxi_lpd_aclk] [get_bd_pins ps_e/saxihp3_fpd_aclk]
    connect_bd_net [get_bd_pins proc_sys_reset_${default_clk_num}/interconnect_aresetn] [get_bd_pins axi_smartconnect_lpd/aresetn] [get_bd_pins axi_vip_1/aresetn] [get_bd_pins smartconnect_axifull/aresetn]
    
    connect_bd_net -net proc_sys_reset_2_peripheral_aresetn [get_bd_pins axi_vip_pltf/aresetn] [get_bd_pins axi_vip_0/aresetn] 
    ...
    if {!$use_ddr } {
      ...
      connect_bd_net [get_bd_pins clk_wiz_0/$default_clk_port] [get_bd_pins smartconnect_axihpm0fpd/aclk] [get_bd_pins ps_e/maxihpm0_fpd_aclk]
      connect_bd_net [get_bd_pins proc_sys_reset_${default_clk_num}/interconnect_aresetn] [get_bd_pins smartconnect_axihpm0fpd/aresetn] 
    }
	...
	# assign_bd_address -offset 0xA0000000 -range 0x010000 -target_address_space /ps_e/Data [get_bd_addr_segs axi_vip_pltf/S_AXI/Reg] -force
  ...
  if {$irqs == "32"} {
    set_property PFM.AXI_PORT $parVal [get_bd_cells /smartconnect_axilite] }

  set hp3Val []
  for {set i 1} {$i < 16} {incr i} {
    lappend hp3Val S[format %02d $i]_AXI {memport "S_AXI_HP" sptag "HP3" memory "ps_e HP3_DDR_LOW"}
  }
  set_property PFM.AXI_PORT $hp3Val [get_bd_cells /smartconnect_axifull]

  set lpdVal []
  for {set i 1} {$i < 16} {incr i} {
    lappend lpdVal S[format %02d $i]_AXI {memport "S_AXI_HP" sptag "LPD" memory "ps_e LPD_DDR_LOW"}
  }
  set_property PFM.AXI_PORT $lpdVal [get_bd_cells /axi_smartconnect_lpd]

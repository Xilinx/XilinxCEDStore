...
proc getSupportedBoards {} {
	...
	set V_board_unique [get_board_parts -filter {(BOARD_NAME =~ "*vek385*" || BOARD_NAME =~ "*vmk365*") && VENDOR_NAME == "xilinx.com"} -latest_file_version]
...
proc addOptions {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
    ...
    # NEW: Stream source selection for DC_Bypass mode
    lappend x [dict create name "Stream_Source" type "string" value "AVTPG" value_list {"AVTPG" "DDR_(SST)" "AVTPG_and_DDR_(MST)"} enabled true]
...
proc addGUILayout {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
	...
	ced::add_param -name MMI_Config -display_name "MMI Configuration" -parent $left_panel -designObject $designObj -widget comboBox
        ...
        # NEW: Stream source selection
        ced::add_param -name Stream_Source -display_name "Stream Source" -parent $right_panel -designObject $designObj -widget comboBox
...
gui_updater {PROJECT_PARAM.BOARD_PART MMI_Config.VALUE DPDC_Presentation_Mode.VALUE Video_Streams.VALUE Video_Interface.VALUE MST_ENABLE.VALUE Stream_Source.VALUE} {DPDC_Presentation_Mode.VISIBLE Image.IMAGE_PATH} {
...
set DPDC_Config.DISPLAYNAME "MMI DC Configurations"
set DPDC_Presentation_Mode.VISIBLE false
if { [regexp "vmk365" ${PROJECT_PARAM.BOARD_PART}] } {
set Image.IMAGE_PATH "gpu_dual_display_vmk365.png"
} else {
...
} elseif { ${Stream_Source.VALUE} == "DDR_(SST)" } {
    # Single stream from DDR
    set Image.IMAGE_PATH "bypass_ddr_single.png"
} else {
    # AVTPG_and_DDR_(MST) - 2 streams (1 DDR + 1 AVTPG)
    set Image.IMAGE_PATH "bypass_ddr_hybrid.png"
}
} else {
set DPDC_Config.DISPLAYNAME "MMI DC Configurations"
...
# NEW: Control Stream_Source visibility - only show in DC_Bypass mode
gui_updater {PROJECT_PARAM.BOARD_PART MMI_Config.VALUE} {Stream_Source.VISIBLE Stream_Source.VALUE} {
if { ${MMI_Config.VALUE} == "DC_Bypass" } {
set Stream_Source.VISIBLE true
} else {
set Stream_Source.VALUE "AVTPG"
set Stream_Source.VISIBLE false
}
}

# MODIFIED: MST_ENABLE visibility - only show when DC_Bypass AND AVTPG is selected
gui_updater {PROJECT_PARAM.BOARD_PART MMI_Config.VALUE Stream_Source.VALUE} {MST_ENABLE.VALUE MST_ENABLE.VISIBLE} {
if { ${MMI_Config.VALUE} == "DC_Bypass" && ${Stream_Source.VALUE} == "AVTPG" } {
...
# MODIFIED: Video_Streams visibility - only show when DC_Bypass AND AVTPG AND MST enabled
gui_updater {PROJECT_PARAM.BOARD_PART MMI_Config.VALUE Stream_Source.VALUE MST_ENABLE.VALUE} {Video_Streams.VISIBLE} {
if { ${MMI_Config.VALUE} == "DC_Bypass" && ${Stream_Source.VALUE} == "AVTPG" && ${MST_ENABLE.VALUE} == "true" } {

...
proc create_quad {parentCell nameHier refclk_name gt_type line_rate refclk_freq {share_refclk 0}} {
    # The quad is built IBERT-style (as in versal_ibert_ced): gtwiz_versal drives
    # the GT directly and there is no prbs_generator_checker "bridge" IP. The GT
    # line-rate/refclk settings that used to be pushed into the bridge via
    # CONFIG.IP_LR0_SETTINGS are applied straight to gtwiz_versal through
    # CONFIG.INTF0_GT_SETTINGS(LR0_SETTINGS), and the INTF0_TX*/RX*_GT_IP_Interface
    # pins are intentionally left unconnected.
    #
    ...
    } elseif {$gt_type eq "GTM2"} {
	if {!$share_refclk} {
	  create_bd_cell -type ip -vlnv xilinx.com:ip:util_ds_buf util_ds_buf
	  set_property CONFIG.C_BUF_TYPE {IBUFDS_GTM2} [get_bd_cells util_ds_buf]
	  make_bd_intf_pins_external  [get_bd_intf_pins util_ds_buf/CLK_IN_D3]
	  #set_property name gt_refclk_$name [get_bd_intf_ports CLK_IN_D3_0]
	}
        set width [expr $line_rate > 57.0 ? 320 : 160]
        set user_settings_dict [dict create TX_LINE_RATE $line_rate TX_REFCLK_FREQUENCY $refclk_freq RX_LINE_RATE $line_rate RX_REFCLK_FREQUENCY $refclk_freq GT_TYPE GTM2 TX_USER_DATA_WIDTH $width RX_USER_DATA_WIDTH $width TX_OUTCLK_SOURCE TXPROGDIVCLK RX_OUTCLK_SOURCE RXPROGDIVCLK]
    } else {
	if {!$share_refclk} {
	  create_bd_cell -type ip -vlnv xilinx.com:ip:util_ds_buf util_ds_buf
   ...
   create_bd_cell -type ip -vlnv xilinx.com:ip:bufg_gt bufg_gt_rx
	...
	connect_bd_net [get_bd_pins bufg_gt_tx/usrclk] [get_bd_pins gtwiz_versal/QUAD0_TX0_usrclk] [get_bd_pins gtwiz_versal/QUAD0_TX1_usrclk] [get_bd_pins gtwiz_versal/QUAD0_TX2_usrclk] [get_bd_pins gtwiz_versal/QUAD0_TX3_usrclk]
	connect_bd_net [get_bd_pins bufg_gt_rx/usrclk] [get_bd_pins gtwiz_versal/QUAD0_RX0_usrclk] [get_bd_pins gtwiz_versal/QUAD0_RX1_usrclk] [get_bd_pins gtwiz_versal/QUAD0_RX2_usrclk] [get_bd_pins gtwiz_versal/QUAD0_RX3_usrclk]
	...
	make_bd_intf_pins_external  [get_bd_intf_pins gtwiz_versal/Quad0_GT_Serial]
        ...
        if {!$share_refclk} {
          if {$gt_type eq "GTM"} {
            ...
            connect_bd_net [get_bd_pins util_ds_buf/IBUFDS_GTM2_O] [get_bd_pins gtwiz_versal/QUAD0_GTREFCLK0]
	  } else {
    ...
    if {!$share_refclk} {
      if {$gt_type eq "GTM"} {
        ...
        set_property name bridge_refclk${refclk_name}_diff_gt_ref_clock [get_bd_intf_ports CLK_IN_D3_0]
      } else {
    ...
    if {$share_refclk} {
      ...
      group_bd_cells $nameHier [get_bd_cells bufg_gt_tx] [get_bd_cells gtwiz_versal] [get_bd_cells bufg_gt_rx]
    } elseif {$gt_type eq "GTM"} {
      group_bd_cells $nameHier [get_bd_cells bufg_gt_tx] [get_bd_cells gtwiz_versal] [get_bd_cells bufg_gt_rx] [get_bd_cells util_ds_buf] [get_bd_cells xlconstant]
    } else {
      group_bd_cells $nameHier [get_bd_cells bufg_gt_tx] [get_bd_cells gtwiz_versal] [get_bd_cells bufg_gt_rx] [get_bd_cells util_ds_buf]
    }
    if {$share_refclk} {
    ...
    connect_bd_net [get_bd_pins apb3clk] [get_bd_pins gtwiz_versal/gtwiz_freerun_clk]

	...
	proc create_root_design { currentDir design_name temp_options} {
		...
		if {([lsearch $temp_options Preset.VALUE] == -1) || ([lsearch $temp_options PS7_Only] != -1)} {
			...
			if { $led_board_interface != "" } {
				...
				apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config { Clk_master {/processing_system7_0/FCLK_CLK0 (50 MHz)} Clk_slave {Auto} Clk_xbar {Auto} Master {/processing_system7_0/M_AXI_GP0} Slave {/axi_gpio_0/S_AXI} ddr_seg {Auto} intc_ip {New AXI SmartConnect} master_apm {0}}  [get_bd_intf_pins axi_gpio_0/S_AXI]
			}

================================================================================
  Versal CPM6 DMA Gen6 PIPE Simulation -- CTRL1 EP (no DDR)
  Controller: CTRL1 only (Endpoint)
  Mode      : PIPE Simulation
================================================================================

Steps to run CPM6 DMA (CTRL1) simulation
...
5.  Create the Vivado project from this CED.
    ...
    and set the clibs path in sim settings and generate scripts enabled
...
cpm6/ctrl1ep_g6x8_hdma_1pf). This variant (hdma, non-DDR/PL-AXI-backed)
should use the "plaxi" flavored tests:

  hello_world                                (generic framework smoke test, default)
  test_s_hdma_plaxi_ctrlr1
  test_M_bridge_plaxi_ctrlr1
  test_M_bridge_plaxi_ctrlr1_4pf
  test_M_bridge_plaxi_ctrlr1_4pf_axildecode
...
sign-off. The "ddr" flavored tests (for the hdma_ddr variant) are also
compiled in (all HDMA tests share one test_pkg.svh) but are expected to
target hdma_ddr's LPDDR5-backed hardware, not this variant.

...
// cdo_loader_sim_if.sv is a generic interface type definition needed by
// the framework regardless of flow (e.g. shim_vsequencer.sv unconditionally
// declares a `virtual cdo_loader_sim_if` handle) -- only its functional
// *use* is CPM6_RTL-specific (gated in test_base.sv/shim_vsequencer.sv),
// per uvma-pcie-sim-framework/cpm6/common/tb/tb_top.sv, which includes it
// unconditionally outside its `ifdef CPM6_RTL/`elsif CPM6_VIVADO block.
...
module tb_top;
  ...
  // ------------------------------------------------------------------ //
  parameter LINK0_WIDTH = 0; // max=8; 0 means disable
  parameter LINK1_WIDTH = 8; // max=8; 0 means disable
  // ------------------------------------------------------------------ //

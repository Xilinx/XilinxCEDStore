class test_s_dma_aximm extends test_dma_tasks;
//class test_s_dma_aximm extends test_basic;

  `uvm_component_utils(test_s_dma_aximm);
...
//   rc_mem_callback mem_cb;
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
    ...
    for (int ii=0; ii<2; ii++) begin
      ...
      vip_cfg.pcie_cfg[ii].flit_mode_ctl = dut_ctrlr_en[ii];
      ...
      dut_cfg.pcie_cfg[ii].flit_mode_ctl = dut_ctrlr_en[ii];
    end
  endfunction
...
//     chn_num  = 1;
    `uvm_info("---- Simple DMA to AXI0 ---- ", $sformatf("Channel number %d, Size : 0x%d",chn_num, txfr_size), UVM_NONE)
      ...
      s_dma_h2c_transfer_start(chn_num, host_addr, 64'h050000010000, txfr_size, irq);

      // C2H on Channel 4 port 0 
      s_dma_c2h_transfer_start(chn_num, 64'h050000010000, host_addr, txfr_size, irq);
/*
     // AXI port 1
      ...
      s_dma_h2c_transfer_start(chn_num, host_addr, 64'h050000020000, txfr_size, irq);

      // C2H on Channel 5 port 1 
      s_dma_c2h_transfer_start(chn_num, 64'h050000020000, host_addr, txfr_size, irq);
     ...
     // AXI port 2
      ...
      s_dma_h2c_transfer_start(chn_num, host_addr, 64'h050000040000, txfr_size, irq);

      // C2H on Channel 7 port 2 
      s_dma_c2h_transfer_start(chn_num, 64'h050000040000, host_addr, txfr_size, irq);
     ...
     // AXI port 3
      ...
      // H2C on Channel 6 port 2
      s_dma_h2c_transfer_start(chn_num, host_addr, 64'h050000060000, txfr_size, irq);

      // C2H on Channel 7 port 2 
      s_dma_c2h_transfer_start(chn_num, 64'h050000060000, host_addr, txfr_size, irq);

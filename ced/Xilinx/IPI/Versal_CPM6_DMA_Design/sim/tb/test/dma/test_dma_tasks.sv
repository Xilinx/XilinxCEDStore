...
class test_dma_tasks extends base_ep_test;
//class test_dma_tasks extends test_basic;

  `uvm_component_utils(test_dma_tasks)
...
// H2C transfers task Read channel.
  virtual task s_dma_h2c_transfer_start(
               bit [5:0]  chn,
	       bit [63:0] src_addr,
	       bit [63:0] dst_addr,
	       bit [31:0] size,
...
// C2H transfers task Write channel.
  virtual task s_dma_c2h_transfer_start(
      ...
      s_dma_chk_txfr_done(C2H, wr_chn_base_addr + 16'h1c, msi_int, msix_int);
      `uvm_info("-- C2H Simple DMA transfer Completed. ", $sformatf("Chan = %d. Chn_base_addr = 0x%h", chn, wr_chn_base_addr), UVM_NONE)
     
  endtask // s_dma_c2h_transfer_start

  virtual task s_dma_chk_txfr_done(dma_type_t typ, bit [31:0]  adr, bit msi_int, bit msix_int);
    logic [63:0] addr;
    logic [31:0] data_arr[];
    logic [31:0] data;
    logic [5:0]  count;
  ...
  endtask // s_dma_chk_txfr_done

    virtual task dma_chk_txfr_done(dma_type_t dir,
				  bit [15:0]  chn_addr);
        logic [63:0] addr;
        logic [31:0] data_arr[];
        logic [31:0] data;
...
//        addr = env.pcie_sts.pdevs[1].mem_bar_q[0].base + chn_addr + 16'h80;  // 0x80 status reg offset for a given channel
  ...
  virtual task gen_ll_dsc(dma_type_t dir,
     ...
     dsc_bar = dsc_port +1;  // BAR 0 is used for DMA. BAR1 -> PL_AXI_0, BAR2 -> PL_AXI1, BAR3 -> PL_AXI_2, BAR4 -> PL_AXI3, BAR5 -> PS0  
 ...
 // task for contest write in link linst mode
  virtual task dma_ll_ctext_program(dma_type_t dir,
  ...
  endtask // dma_ll_ctext_program

/************************************************************
Task : dma_ctxt_init_prog
Inputs : None
Outputs : None
...
*************************************************************/
task dma_ctxt_init_prog (int num_chn);
  integer i;
  begin

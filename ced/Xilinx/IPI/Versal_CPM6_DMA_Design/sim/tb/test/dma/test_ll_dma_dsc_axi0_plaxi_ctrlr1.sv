...
class test_ll_dma_dsc_axi0_plaxi_ctrlr1 extends test_dma_tasks;

  `uvm_component_utils(test_ll_dma_dsc_axi0_plaxi_ctrlr1)
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
   ...
   virtual task main_phase(uvm_phase phase);
	 ...
	 `uvm_info("***** Start DMA LL transfer parameters", $sformatf("Chan = %d. data port select = %d, DMA read_base_addr = 0x%h, DMA write_base_addr = 0x%h, transfer size = %d, h2c_src_addr = 0x%h, h2c_dst_address = 0x%h, c2h_src_addr = 0x%h, c2h_dst_addr 0x%h", chn, port_sel, rd_chn_base_addr, wr_chn_base_addr, txfr_size, h2c_src_address, h2c_dst_address, c2h_src_address, c2h_dst_address), UVM_NONE)
	 ...
	 dma_ll_ctext_program(H2C, rd_chn_base_addr, desc_addr_h2c, tot_dsc);   // H2C desc location at PS_axi0
	 
	 dma_chk_txfr_done(H2C, rd_chn_base_addr);
	 `uvm_info("-- H2C LL DMA transfer Completed. ", $sformatf("Chan = %d. Chn_base_addr = 0x%h", chn, rd_chn_base_addr), UVM_NONE)

	 dma_ll_ctext_program(C2H, wr_chn_base_addr, desc_addr_c2h, tot_dsc);  // C2H desc location at PS_axi0 +0x1000
	 
	 dma_chk_txfr_done(C2H, wr_chn_base_addr);

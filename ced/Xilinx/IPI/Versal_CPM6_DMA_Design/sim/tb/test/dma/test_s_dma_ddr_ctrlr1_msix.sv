/*
Test case : test_s_dma_ddr_ctrlr1_msix
description:  This test case is to verify basic functionality of DMA with DDR controller 1 
              as target and MSIX interrupt.  This test will do simple H2C and C2H transfer to 
              DDR and check for data integrity.  MSIX interrupt will be generated for each transfer 
              and checked in the test.
              This test uses only PF0 function for all DMA transfers.

*/

class test_s_dma_ddr_ctrlr1_msix extends test_dma_tasks;

  `uvm_component_utils(test_s_dma_ddr_ctrlr1_msix);
...
//   rc_mem_callback mem_cb;
  ...
  virtual task main_phase(uvm_phase phase);
    ...
    // Program MSIX table for all function that are enabled.  This is needed for MSIX interrupt test.
    ...
    for (int pf_num = 0; pf_num < num_pfs_cfg; pf_num++) begin
      TSK_PROGRAM_MSIX_VEC_TABLE(pf_num[7:0]);
    end

    // Program DMA context for number of channels. thiw will program only address offset 0x0 and 0x88 for each channel, which is used for interrupt setup in this test.
    dma_ctxt_init_prog(4);
 ...
 // Interrupt setup
  ...
  // 
    ...
    for (int ii=0; ii<10; ii++) begin 
      ...
      s_dma_h2c_transfer_start(chn_num, host_addr, fpga_dev_addr, txfr_size, msi_int, msix_int);

     // C2H transfer to AXI port 0 
      s_dma_c2h_transfer_start(chn_num, fpga_dev_addr, host_addr, txfr_size, msi_int, msix_int);

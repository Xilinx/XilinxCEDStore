class test_ll_dma extends test_dma_tasks;

  `uvm_component_utils(test_ll_dma)
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
   ...
   virtual task main_phase(uvm_phase phase);
	...
	default : begin h2c_dst_address = axi_port0;  c2h_src_address = axi_port0; end
      ...
      `uvm_info("***** Start DMA LL transfer parameters", $sformatf("Chan = %d. port select = %d, DMA read_base_addr = 0x%h, DMA write_base_addr = 0x%h, transfer size = %d, h2c_src_addr = 0x%h, h2c_dst_address = 0x%h, c2h_src_addr = 0x%h, c2h_dst_addr 0x%h", chn, port_sel, rd_chn_base_addr, wr_chn_base_addr, txfr_size, h2c_src_address, h2c_dst_address, c2h_src_address, c2h_dst_address), UVM_NONE)
      ...
      s_dma_chk_txfr_done(H2C, rd_chn_base_addr + 16'h1c);
      ...
      s_dma_chk_txfr_done(C2H, wr_chn_base_addr + 16'h1c);

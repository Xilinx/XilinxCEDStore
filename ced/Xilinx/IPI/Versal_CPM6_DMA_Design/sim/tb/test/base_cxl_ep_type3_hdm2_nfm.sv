 ...
 * non-flit mode and the EP advertises 2 HDM ranges.
 * EXTENDS
 * base_cxl_ep_test
 * ACRONYMS
...
class base_cxl_ep_type3_hdm2_nfm extends base_cxl_ep_test;

  `uvm_component_utils(base_cxl_ep_type3_hdm2_nfm)

  // CXL-specific mailbox
  cseq_cxl_mbox cxl_mbox[0:1];

  // Ensure link comes up in non-flit mode
  rand bit [1:0] vip_fm; 
  rand bit [1:0] dut_fm; 
...
`endif
  ...
  virtual function void end_of_elaboration_phase(uvm_phase phase);
    ...
    // Configure DUT and VIP
    for (int ii=0; ii<2; ii++) begin
      // VIP as RP
      vip_cfg.pcie_cfg[ii].flit_mode_ctl = vip_fm[ii];
      // DUT as EP
      /********************************************************/
      /* ONLY APPLICABLE TO RTL SIMS USING BASE+APPENDED CDOs */
      /********************************************************/
      dut_cfg.pcie_cfg[ii].flit_mode_ctl  = dut_fm[ii];
      dut_cfg.cxl_cfg[ii].cxl_device_type = 3;
      // PCIe DVSEC for CXL Devices (set 2 HDM range)
      dut_cfg.cxl_cfg[ii].dvsec_cxl_cap.cxl_cap.hdm_count = 2; 
      // CXL HDM Decoder Capability (via ELBI) ; offset at 0x200
      dut_cfg.cxl_cfg[ii].hdm_dec_cap.en = 1'b1;
      dut_cfg.cxl_cfg[ii].hdm_dec_cap.cap.decoder_cnt          = 1;     //2 Decoder
      dut_cfg.cxl_cfg[ii].hdm_dec_cap.cap.coherency_model_supp = 2'b10; //HDM-H
      dut_cfg.cxl_cfg[ii].hdm_dec_cap.decoder = new[2];
    ...
    env.shim.setup_vip_nfm(); //we know link will operate in non-flit mode

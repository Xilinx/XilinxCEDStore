...
class test_M_bridge_ddr_ctrlr1_bar24_1pf extends test_hdma_tasks;

  `uvm_component_utils(test_M_bridge_ddr_ctrlr1_bar24_1pf);
  ...
  for (int i=0; i <10; i++) begin     
	   ...
	   `uvm_info("***", $sformatf("bar = %d, reg addr : 0x%h, data = 0x%h",2, bar2_addr,bar2_data), UVM_NONE) 
	   bar4_addr = $urandom_range(0, 32'h1ffff);
	   `uvm_info("***", $sformatf("bar = %d, reg addr : 0x%h, data = 0x%h",4, bar4_addr,bar4_data), UVM_NONE)
	  
	   // BAR Write
	   bar_write   (pf, 2, bar2_addr, num_byt2, bar2_data);  //PLAXI0
	   bar_write   (pf, 4, bar4_addr, num_byt4, bar4_data);  // NOC DDR0
	   
	   // BAR Read
	   bar_read_chk(pf, 2, bar2_addr, num_byt2, bar2_data); // PLAXI0
	   bar_read_chk(pf, 4, bar4_addr, num_byt4, bar4_data); // NOC DDR0

	   
	end // for (int i=0; i <10; i++)
	pf = pf+1; // PF0 to PF4
     end // for (int ii=0; ii < num_pfs_cfg; ii++)

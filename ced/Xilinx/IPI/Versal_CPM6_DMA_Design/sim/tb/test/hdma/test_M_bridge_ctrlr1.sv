...
class test_M_bridge_ctrlr1 extends test_hdma_tasks;

  `uvm_component_utils(test_M_bridge_ctrlr1);
  ...
  virtual task main_phase(uvm_phase phase);
	...
	bar1_addr = $urandom_range(32'h10000, 32'h1ffff);
...
//	bar4_addr[1:0] = 0;
	bar5_addr = $urandom_range(0, 32'h0ffff);  // DDR is split into 2 128K
	`uvm_info("***", $sformatf("bar = %d, reg addr : 0x%h, data = 0x%h",1, bar5_addr,bar5_data), UVM_NONE)
//	bar5_addr[1:0] = 0;
	
	// BAR Write
	bar_write   (pf, 1, bar1_addr, bar1_data);
	bar_write   (pf, 2, bar2_addr, bar2_data);
	bar_write   (pf, 3, bar3_addr, bar3_data);
	bar_write   (pf, 4, bar4_addr, bar4_data);
	bar_write   (pf, 5, bar5_addr, bar5_data);  

	// BAR Read
	bar_read_chk(pf, 1, bar1_addr, bar1_data);
	bar_read_chk(pf, 2, bar2_addr, bar2_data);
	bar_read_chk(pf, 3, bar3_addr, bar3_data);
	bar_read_chk(pf, 4, bar4_addr, bar4_data);
	bar_read_chk(pf, 5, bar5_addr, bar5_data);

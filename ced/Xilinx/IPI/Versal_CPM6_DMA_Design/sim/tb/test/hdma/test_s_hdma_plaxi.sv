class test_s_hdma_aximm extends test_hdma_tasks;
//class test_s_hdma_aximm extends test_basic;

  `uvm_component_utils(test_s_hdma_aximm);
...
//     chn_num  = 1;
    `uvm_info("---- Simple DMA to AXI0 ---- ", $sformatf("Channel number %d, Size : 0x%d",chn_num, txfr_size), UVM_NONE)
      ...
      s_hdma_h2c_transfer_start(chn_num, host_addr, 64'h050000010000, txfr_size, irq);

      // C2H on Channel 4 port 0 
      s_hdma_c2h_transfer_start(chn_num, 64'h050000010000, host_addr, txfr_size, irq);
/*
     // AXI port 1
      ...
      s_hdma_h2c_transfer_start(chn_num, host_addr, 64'h050000020000, txfr_size, irq);

      // C2H on Channel 5 port 1 
      s_hdma_c2h_transfer_start(chn_num, 64'h050000020000, host_addr, txfr_size, irq);
     ...
     // AXI port 2
      ...
      s_hdma_h2c_transfer_start(chn_num, host_addr, 64'h050000040000, txfr_size, irq);

      // C2H on Channel 7 port 2 
      s_hdma_c2h_transfer_start(chn_num, 64'h050000040000, host_addr, txfr_size, irq);
     ...
     // AXI port 3
      ...
      // H2C on Channel 6 port 2
      s_hdma_h2c_transfer_start(chn_num, host_addr, 64'h050000060000, txfr_size, irq);

      // C2H on Channel 7 port 2 
      s_hdma_c2h_transfer_start(chn_num, 64'h050000060000, host_addr, txfr_size, irq);

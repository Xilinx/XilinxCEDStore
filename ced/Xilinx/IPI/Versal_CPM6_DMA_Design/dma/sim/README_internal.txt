================================================================================
  Versal CPM6 DMA Gen6 PIPE Simulation -- INTERNAL README (AMD/XSJ Servers)
  Controller: CTRL1 only (Endpoint), no DDR
...
--------------------------------------------------------------------------------
  ...
  Releases actually exercised against cpm6/ctrl1ep_g6x8_dma_1pf (per real
...
--------------------------------------------------------------------------------
  ...
  In Vivado TCL console:
    set_param ced.repoPaths {<path_to_Versal_CPM6_DMA_Design>}
    open_example_project -force -part xc2vp3602-vsvc3340-2LHP-e-S \
      -dir <output_dir> \
      [get_example_designs -of [get_ipdefs xilinx.com:design:cpm6_dma:1.0]]
    # In the CED options dialog choose CTRL_CONFIG=Controller_1, DDR_EN=false
    set_property compxlib.vcs_compiled_library_dir \
      /proj/xbuilds/2026.1_daily_latest/clibs/vcs/X-2025.06/lin64/lib \
      [current_project]
    launch_simulation -scripts_only
    set_property compxlib.vcs_compiled_library_dir \
      /proj/xbuilds/2026.1.1_daily_latest/clibs/vcs/X-2025.06-SP2/lin64/lib \
      [current_project]
...
--------------------------------------------------------------------------------
  ...
  2. No DMA-specific UVM tests exist yet -- only the generic framework
     hello_world test is verified against this DUT (see
     cpm6/ctrl1ep_g6x8_dma_1pf for a working reference regression run
     of the same dma_ddr sources). sim/test/ still contains the BMD
     example design's verification content and does not apply here.

  3. compile.sh VCS version:

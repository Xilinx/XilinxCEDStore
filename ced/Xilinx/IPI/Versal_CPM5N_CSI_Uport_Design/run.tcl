...
# None
set infile [open [get_files board.v]]
set contents [read $infile]
close $infile
set contents [string map [list "design_1" "$design_name"] $contents]

set outfile  [open [get_files board.v] w]
puts -nonewline $outfile $contents
close $outfile

# None
set infile [open [get_files design_1_wrapper_sim_wrapper.v]]
set contents [read $infile]
close $infile
set contents [string map [list "design_1" "$design_name"] $contents]

set outfile  [open [get_files design_1_wrapper_sim_wrapper.v] w]
puts -nonewline $outfile $contents
close $outfile

# None
set infile [open [get_files axi_vip_master_agent.sv]]
set contents [read $infile]
close $infile
set contents [string map [list "design_1" "$design_name"] $contents]

set outfile  [open [get_files axi_vip_master_agent.sv] w]
puts -nonewline $outfile $contents
close $outfile

# None
set infile [open [get_files sample_tests_1024.vh]]
set contents [read $infile]
close $infile
set contents [string map [list "design_1" "$design_name"] $contents]

set outfile  [open [get_files sample_tests_1024.vh] w]
puts -nonewline $outfile $contents
close $outfile
...
open_bd_design [get_bd_files $design_name]
    ...
    1. Refer to https://github.com/Xilinx/XilinxCEDStore/tree/2023.2/ced/Xilinx/IPI/Versal_CPM5N_CSI_Uport_Design/readme.txt} [current_bd_design]

    regenerate_bd_layout -layout_string {
   "ActiveEmotionalView":"Default View",
   "comment_0":"Next Steps:
    1. Refer to https://github.com/Xilinx/XilinxCEDStore/tree/2023.2/ced/Xilinx/IPI/Versal_CPM5N_CSI_Uport_Design/readme.txt",
   "commentid":"comment_0|",
   "font_comment_0":"18",

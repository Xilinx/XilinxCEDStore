...
################################################################
# This is a generated script based on design: dsp_cips
#
...
variable design_name_dsp_cips
set design_name_dsp_cips dsp_cips

# If you do not already have an existing IP Integrator design open,
# you can create a design using the following command:
#    create_bd_design $design_name_dsp_cips
...
if { ${design_name_dsp_cips} eq "" } {
   # USE CASES:
   #    1) Design_name not set

   set errMsg "Please set the variable <design_name_dsp_cips> to a non-empty value."
   set nRet 1

} elseif { ${cur_design} ne "" && ${list_cells} eq "" } {
   # USE CASES:
   #    2): Current design opened AND is empty AND names same.
   #    3): Current design opened AND is empty AND names diff; design_name_dsp_cips NOT in project.
   #    4): Current design opened AND is empty AND names diff; design_name_dsp_cips exists in project.

   if { $cur_design ne $design_name_dsp_cips } {
      common::send_gid_msg -ssname BD::TCL -id 2001 -severity "INFO" "Changing value of <design_name_dsp_cips> from <$design_name_dsp_cips> to <$cur_design> since current design is empty."
      set design_name_dsp_cips [get_property NAME $cur_design]
   }
   common::send_gid_msg -ssname BD::TCL -id 2002 -severity "INFO" "Constructing design in IPI design <$cur_design>..."

} elseif { ${cur_design} ne "" && $list_cells ne "" && $cur_design eq $design_name_dsp_cips } {
   # USE CASES:
   #    5) Current design opened AND has components AND same names.

   set errMsg "Design <$design_name_dsp_cips> already exists in your project, please set the variable <design_name_dsp_cips> to another value."
   set nRet 1
} elseif { [get_files -quiet ${design_name_dsp_cips}.bd] ne "" } {
   # USE CASES: 
   #    6) Current opened design, has components, but diff names, design_name_dsp_cips exists in project.
   #    7) No opened design, design_name_dsp_cips exists in project.
   ...
   # USE CASES:
   #    8) No opened design, design_name_dsp_cips not in project.
   #    9) Current opened design, has components, but diff names, design_name_dsp_cips not in project.

   common::send_gid_msg -ssname BD::TCL -id 2003 -severity "INFO" "Currently there is no design <$design_name_dsp_cips> in project, so creating one..."

   create_bd_design $design_name_dsp_cips

   common::send_gid_msg -ssname BD::TCL -id 2004 -severity "INFO" "Making design <$design_name_dsp_cips> as current_bd_design."
   current_bd_design $design_name_dsp_cips

}

common::send_gid_msg -ssname BD::TCL -id 2005 -severity "INFO" "Currently the variable <design_name_dsp_cips> is equal to \"$design_name_dsp_cips\"."
...
proc create_root_design { parentCell } {
  ...
  set_property -dict [list \
    CONFIG.CPM_CONFIG { \
      ...
      CPM_PCIE0_PORT_TYPE {Switch_Port-Downstream} \
  ...
  connect_bd_intf_net -intf_net axis_register_slice_2_M_AXIS [get_bd_intf_pins versal_cips_0/pcie0_s_axis_rq] [get_bd_intf_pins axis_register_slice_2/M_AXIS]
  connect_bd_intf_net -intf_net axis_register_slice_3_M_AXIS [get_bd_intf_pins versal_cips_0/pcie0_s_axis_cc] [get_bd_intf_pins axis_register_slice_3/M_AXIS]
  ...
  connect_bd_intf_net -intf_net pcie0_s_axis_cc_0_1 [get_bd_intf_ports pcie0_s_axis_cc_0] [get_bd_intf_pins axis_register_slice_3/S_AXIS]
  connect_bd_intf_net -intf_net pcie0_s_axis_rq_0_1 [get_bd_intf_ports pcie0_s_axis_rq_0] [get_bd_intf_pins axis_register_slice_2/S_AXIS]
  ...
  connect_bd_intf_net -intf_net versal_cips_0_pcie0_m_axis_rc [get_bd_intf_pins versal_cips_0/pcie0_m_axis_rc] [get_bd_intf_pins axis_register_slice_0/S_AXIS]
  ...
  connect_bd_net -net versal_cips_0_pcie0_user_clk [get_bd_pins versal_cips_0/pcie0_user_clk] [get_bd_ports pcie0_user_clk_0] [get_bd_pins axis_register_slice_0/aclk] [get_bd_pins axis_register_slice_1/aclk] [get_bd_pins axis_register_slice_2/aclk] [get_bd_pins axis_register_slice_3/aclk]
  ...
  connect_bd_net -net ilconstant_0_dout [get_bd_pins ilconstant_0/dout] [get_bd_pins axis_register_slice_1/aresetn] [get_bd_pins axis_register_slice_0/aresetn] [get_bd_pins axis_register_slice_2/aresetn] [get_bd_pins axis_register_slice_3/aresetn]

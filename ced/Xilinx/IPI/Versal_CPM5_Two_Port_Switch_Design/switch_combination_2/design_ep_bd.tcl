...
#    create_bd_design $design_name_ep
...
if { ${design_name_ep} eq "" } {
   # USE CASES:
   #    1) Design_name not set

   set errMsg "Please set the variable <design_name_ep> to a non-empty value."
   set nRet 1

} elseif { ${cur_design} ne "" && ${list_cells} eq "" } {
   # USE CASES:
   #    2): Current design opened AND is empty AND names same.
   #    3): Current design opened AND is empty AND names diff; design_name_ep NOT in project.
   #    4): Current design opened AND is empty AND names diff; design_name_ep exists in project.

   if { $cur_design ne $design_name_ep } {
      common::send_gid_msg -ssname BD::TCL -id 2001 -severity "INFO" "Changing value of <design_name_ep> from <$design_name_ep> to <$cur_design> since current design is empty."
      set design_name_ep [get_property NAME $cur_design]
   }
   common::send_gid_msg -ssname BD::TCL -id 2002 -severity "INFO" "Constructing design in IPI design <$cur_design>..."

} elseif { ${cur_design} ne "" && $list_cells ne "" && $cur_design eq $design_name_ep } {
   # USE CASES:
   #    5) Current design opened AND has components AND same names.

   set errMsg "Design <$design_name_ep> already exists in your project, please set the variable <design_name_ep> to another value."
   set nRet 1
} elseif { [get_files -quiet ${design_name_ep}.bd] ne "" } {
   # USE CASES: 
   #    6) Current opened design, has components, but diff names, design_name_ep exists in project.
   #    7) No opened design, design_name_ep exists in project.
   ...
   # USE CASES:
   #    8) No opened design, design_name_ep not in project.
   #    9) Current opened design, has components, but diff names, design_name_ep not in project.

   common::send_gid_msg -ssname BD::TCL -id 2003 -severity "INFO" "Currently there is no design <$design_name_ep> in project, so creating one..."

   create_bd_design $design_name_ep

   common::send_gid_msg -ssname BD::TCL -id 2004 -severity "INFO" "Making design <$design_name_ep> as current_bd_design."
   current_bd_design $design_name_ep

}

common::send_gid_msg -ssname BD::TCL -id 2005 -severity "INFO" "Currently the variable <design_name_ep> is equal to \"$design_name_ep\"."

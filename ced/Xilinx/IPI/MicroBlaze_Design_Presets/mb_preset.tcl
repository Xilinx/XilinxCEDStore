    ...
    proc create_root_design { parentCell design_name temp_options} {
        ...
        if {([lsearch $temp_options Preset.VALUE] == -1) || ([lsearch $temp_options "Microcontroller"] != -1)} {
            ...
            if {[regexp scu35 $board_name]} {
                ...
                # SCU35 board.xml 2.1 renamed the reset board interface reset->resetn (active-low)
                # and clk_wiz's own CONFIG.RESET_PORT is set to resetn by the board's
                # sys_diff_clock_preset, so the pin is literally named "resetn" now, not "reset".
                # Resolve both the interface and the pin dynamically instead of the old hardcoded
                # "reset ( FPGA Reset )" / "clk_wiz_1/reset", which no longer match on this board.
                set scu35_reset_ifc ""
                catch {
                    set scu35_reset_candidates [get_board_component_interfaces \
                        -filter {BUSDEF_NAME == reset_rtl && NAME !~ *phy* && NAME !~ *pcie* && NAME !~ *dummy*}]
                    if {[llength $scu35_reset_candidates] > 0} { set scu35_reset_ifc [lindex $scu35_reset_candidates 0] }
                }
                if {$scu35_reset_ifc ne ""} {
                    set clkwiz_reset_pin [get_bd_pins -quiet clk_wiz_1/reset]
                    if {$clkwiz_reset_pin eq ""} { set clkwiz_reset_pin [get_bd_pins -quiet clk_wiz_1/resetn] }
                    apply_bd_automation -rule xilinx.com:bd_rule:board -config [list Board_Interface [list $scu35_reset_ifc] Manual_Source {Auto}] $clkwiz_reset_pin
                    apply_bd_automation -rule xilinx.com:bd_rule:board -config [list Board_Interface [list $scu35_reset_ifc] Manual_Source {Auto}] [get_bd_pins rst_clk_wiz_1_100M/ext_reset_in]
                }
            ...
            #   set_property -dict [list CONFIG.CLKOUT2_USED {true} CONFIG.CLKOUT2_REQUESTED_OUT_FREQ {50} CONFIG.MMCM_CLKOUT1_DIVIDE {20} CONFIG.NUM_OUT_CLKS {2}] [get_bd_cells clk_wiz_1]
...
#                 puts "#***********************************Debug: Default Board interfaces Added**************************************#"
            ...
            if {[regexp scu35 $board_name]} {
                puts "INFO:: SCU35 Board Selected"
                puts "ERROR: Real-time_Processor selected and not supported for SCU35 Board"

            } else {

                if { $mem_ctrl != "ddr4" } {

                    puts "INFO: Running DDR3 configurations"
                    ...
                    if { $ddr4_board_interface_1 != "" } {
                        ...
                        puts "INFO: Running DDR4 configurations"
                        ...
                        puts "INFO: Adding AXI-BRAM configurations"
                        ...
                        # create_bd_cell -type ip -vlnv xilinx.com:ip:axi_interconnect axi_smartconnect_0
                        ...
                        connect_bd_intf_net [get_bd_intf_pins microblaze_0/M_AXI_DC] -boundary_type upper [get_bd_intf_pins axi_smartconnect_0/S00_AXI]
                        connect_bd_intf_net [get_bd_intf_pins microblaze_0/M_AXI_IC] -boundary_type upper [get_bd_intf_pins axi_smartconnect_0/S01_AXI]
                        ...
                        # connect_bd_net [get_bd_pins axi_smartconnect_0/aclk] [get_bd_pins clk_wiz_1/clk_out1]
                        ...
                        apply_bd_automation -rule xilinx.com:bd_rule:axi4 -config { Clk_master {/clk_wiz_1/clk_out1 (100 MHz)} Clk_slave {Auto} Clk_xbar {/clk_wiz_1/clk_out1 (100 MHz)} Master {/microblaze_0 (Cached)} Slave {/axi_bram_ctrl_0/S_AXI} ddr_seg {Auto} intc_ip {/axi_smartconnect_0} master_apm {0}}  [get_bd_intf_pins axi_bram_ctrl_0/S_AXI]
            ...
            if {[regexp scu35 $board_name]} {
                ...
                puts "INFO:: SCU35 Board Selected"
                puts "ERROR: Application_Processor selected and not supported for SCU35 Board"
                ...
                if {($ddr3_board_interface_1 == "") && ($ddr4_board_interface_1 == "")} {
                    ...
                    # apply_bd_automation -rule xilinx.com:bd_rule:clkrst -config { Clk {/clk_wiz_1/clk_out1 (100 MHz)} Freq {100} Ref_Clk0 {} Ref_Clk1 {} Ref_Clk2 {}}  [get_bd_pins axi_smartconnect_0/S01_ACLK]
                    # apply_bd_automation -rule xilinx.com:bd_rule:clkrst -config { Clk {/clk_wiz_1/clk_out1 (100 MHz)} Freq {100} Ref_Clk0 {} Ref_Clk1 {} Ref_Clk2 {}}  [get_bd_pins axi_smartconnect_0/S02_ACLK]
                    # apply_bd_automation -rule xilinx.com:bd_rule:clkrst -config { Clk {/clk_wiz_1/clk_out1 (100 MHz)} Freq {100} Ref_Clk0 {} Ref_Clk1 {} Ref_Clk2 {}}  [get_bd_pins axi_smartconnect_0/S03_ACLK]
                    # apply_bd_automation -rule xilinx.com:bd_rule:clkrst -config { Clk {/clk_wiz_1/clk_out1 (100 MHz)} Freq {100} Ref_Clk0 {} Ref_Clk1 {} Ref_Clk2 {}}  [get_bd_pins axi_smartconnect_0/S04_ACLK]
                ...
                if {[regexp vcu118 $board_name]||[regexp kc705 $board_name]} {
        ...
        # puts "#***********************************Debug: SCU35 Default Board interfaces Added**************************************#"
        ...
        # puts "#***********************************Debug: SCU35 GPIO Board interfaces Added**************************************#"
        ...
        set_property name microblaze_0_ilconcat [get_bd_cells microblaze_0_xlconcat]
        ...
        # Converting Xilinx utility IPs to inline instances (if present)

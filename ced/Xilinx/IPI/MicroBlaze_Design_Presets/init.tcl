...
proc getSupportedBoards {} {
  # Parenthesized the DISPLAY_NAME OR-chain so VENDOR_NAME/NAME exclusions apply
  # to every family (Kintex/Artix/Virtex), not just Spartan -- without this,
  # kcu1500 (Kintex) and vcu1525 (Virtex) were never excludable here at all,
  # since Tcl && binds tighter than || and the exclusion was nested only under
  # the Spartan clause. kcu1500/vcu1525 are confirmed unsupported by this CED
  # (instantiate_example_design rejects them even though they pass the plain
  # DISPLAY_NAME filter) -- excluded here so the board picker doesn't offer them.
  return [get_board_parts -filter {((DISPLAY_NAME =~"*Kintex*" || DISPLAY_NAME =~"*Artix*" || DISPLAY_NAME =~"*Virtex*" || DISPLAY_NAME =~ "*Spartan*") && VENDOR_NAME=="xilinx.com" && (NAME !~ "*scu200*") && (NAME !~ "*kcu1500*") && (NAME !~ "*vcu1525*"))} -latest_file_version -quiet]
}
...
puts ${PROJECT_PARAM.BOARD_PART}
   ...
   if {[regexp scu35 ${PROJECT_PARAM.BOARD_PART}]} {
   puts "INFO :: Debug - scu35 selected"
...
proc addGUILayout {DESIGNOBJ PROJECT_PARAM.BOARD_PART} {
	...
	set text_panel [ced::add_panel -name Panel -parent $page -designObject $designObj -layout horizontal]
    # Add Text to Display as EOL Warning
	set text "<b style='color:red;'>WARNING: Support for Linux on MicroBlaze is deprecated.
	PetaLinux Microblaze will be end-of-life in the 2026.2 release.</b>"

    set TextVar [ced::add_text -designObject $designObj -name WARNING -tclproc $text -parent $text_panel]
}

gui_updater {PROJECT_PARAM.BOARD_PART Preset.VALUE} {WARNING.VISIBLE} {

  if { ${Preset.VALUE} == "Application_Processor"} {
    set WARNING.VISIBLE true
  } elseif { ${Preset.VALUE} == "Microcontroller"} {
     set WARNING.VISIBLE false

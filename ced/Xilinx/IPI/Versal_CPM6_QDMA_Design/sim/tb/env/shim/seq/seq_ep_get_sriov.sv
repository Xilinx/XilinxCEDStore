...
class seq_ep_get_sriov extends vseq_in_order;
  ...
  virtual task pre_body(); 
    ...
    if (!ecap_sriov_not_found) begin
      ...
      // 0x18 = {VF Device ID, Reserved} -- this is the only place a VF's
      // real Device ID is discoverable; a VF's own Type0 config space is
      // not meant to be probed directly for it (a VF's Vendor ID register
      // there is spec-hardwired to FFFFh). Avery's own auto_enum bus
      // enumeration summary never reads this register either, which is
      // why it prints "vendor_id ffff, device_id ffff" for every VF
      // regardless of what's programmed here -- that print is a VIP
      // display convention, not derived from this field.
      tlp.build_rd(ecap_sriov.base+'h18, .bdf(dst_bdf));
      p_sequencer.api.send_cfg(tlp);
      sriov.vf_device_id = tlp.payload[0][3:2];
  ...
  virtual task post_body();
    ...
    if (print) begin
      ...
      `uvm_info(get_type_name, $sformatf("  - VF Device ID  : 0x%h", sriov.vf_device_id), UVM_LOW)

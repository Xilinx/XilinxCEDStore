    ...
    foreach (mgrs[ii]) begin
      ...
      foreach (mgrs[ii].finfs[jj]) begin
        ...
        if (pdev.ptype==PT_PCIE_EP) begin
          ...
          // VF Extended Capability space only exists once VF Enable is set
          // (SRIOV EC+0x8 bit 0, populated by seq_ep_get_sriov.sv into
          // seq_sriov.sriov.vf_enable). Without this guard, traversing VF
          // capability space while VF Enable=0 gets UR on the config reads,
          // which shim_api.sv escalates to UVM_FATAL during configure_phase.
          if (!seq_sriov.ecap_sriov_not_found && seq_sriov.sriov.vf_enable) begin

class gpmon_mon_cb#(type TXN) extends uvm_callback;

  `uvm_object_param_utils(gpmon_mon_cb#(TXN))

  // uvm*param_utils don't define get_type_name() and type_name
  const static string type_name = {"gpmon_mon_cb#(",
                                   TXN::type_name,
                                   ")"};
  virtual function string get_type_name(); return type_name; endfunction

  function new(string name = "gpmon_mon_cb");
    super.new(name);
  endfunction
